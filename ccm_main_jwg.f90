...
!        print*,"error"
  ...
  case( 'solve_general_EV_test' ) !solve the general eigenvalue problem
...
! jwg end 
  ...
  if ( .not. allocated(ijk_confs)) allocate( ijk_confs(3*nconfs) )
...
!!$  
  ...
  if ( .not. allocated(ijpq_confs)) allocate( ijpq_confs(4*nconfs) )
  ...
  if ( .not. allocated(fock_mtx_3nf)) allocate( fock_mtx_3nf(tot_orbs, tot_orbs) )
  fock_mtx_3nf = 0.d0 
...
!  !$omp parallel do reduction (+: fock_mtx_3nf ) private (ii,kk1,kk2,kk3,kk4,i,j,p,q)
  ...
  ndim2 = tot_orbs**2
  if ( .not. allocated(t2_tmp1)) allocate( t2_tmp1(ndim2) )
  if ( .not. allocated(t2_tmp2)) allocate( t2_tmp2(ndim2) )
  t2_tmp1 = 0.d0
  t2_tmp2 = 0.d0
