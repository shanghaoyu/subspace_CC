...
! for each system (np, pp, nn).
  ...
  real*8,PARAMETER :: d1_plus_d2 = 6.22349655930370016D+00    !5.808457931448150D0   OG
  real*8,PARAMETER :: d3 =  -5.30654650657754967D+00          !-5.684721659000000D0                
  real*8,PARAMETER :: d5 = -4.63631448771476995D-01           !0.052006007530060D0                  
  real*8,PARAMETER :: d14_minus_d15 =-1.09953639023018006D+01 !-11.381880859416331D0  
...
!!$    END IF
  ...
  FUNCTION chp_NNLO_two_pion_exchange_VT(w, A, wt2, impi) RESULT(res)
    ...
    !from  1loop_r_VT
    ![3] D9 = [4] 19 + 22
    res = 0.d0!3.0D0*const(18)*(5.0D0*mpi2(impi) + 2.0D0*q2)*A/mnuc(0)
...
! Calculates all q-independent stuff in the z-integral (z = 2*m_\pi / \mu), i.e. it need only be done once.
  SUBROUTINE chp_calculate_2PE_2loop_int_WC_SFR_data
   ...
   ! if(z_nr .eq. nof_2PE_2loop_int_z_points .and. x_nr .eq. nof_2PE_2loop_int_x_points) print*,'error'
...
! XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxXXX
  ...
  FUNCTION chp_tau1_dot_tauXtau(t1,t2,t3,t4,t5,t6) RESULT(res)
    ...
    ! andreas: t2.t3 = (t2yt3z-t2zt3y, t2zt3x-t2xt3z, t2xt3y-t2yt3x)
...
!! End of 2P-1P contribution to 3NF  

!! Functions for the ring diagrams !! 

function R1_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R1_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  q2 = q22**(1.d0/2.d0)
  ...
  q33 = q32*q3
  impi = 2
  coeff = gA4*gA2/(128.d0*fpi4*fpi2)

  r1_3nf = coeff*(z2-1.d0)*mpi(impi)*( 2.d0*mpi2(impi)+q32 )*( q22*q3 + 4.d0*mpi2(impi)*(z*q1+q3) ) &
       /((4.d0*(z2-1.d0)*mpi2(impi)-q22)*(4.d0*mpi2(impi)*q3+q33)) &
!
       - Aq2*coeff*q22*(2.d0*mpi2(impi)*(q1+z*q3)+z*q3*(q32-q12))/((z2-1.d0)*q1*q32) &
!
       -coeff*Aq3*(z*q22*(z*q1-q3)*q3+2.d0*mpi2(impi)*(z*(z2-2.d0)*q12-(1.d0+z2)*q1*q3-z*q32))&
       /((z2-1.d0)*q1*q3) &
!
       +coeff*Aq1*(2.d0*mpi2(impi)*q22+q3*(-z*q13*q12+(2.d0-3.d0*z2)*q12*q3-z*(z2-2.d0)*q1*q32+q33))&
       /((z2-1.d0)*q32) &
!
       -Int_3pt*q22*4.d0*coeff/((z2-1.d0)*(4.d0*(z2-1.d0)*mpi2(impi)-q22)*q3) &
       *(8.d0*(z2-1.d0)*mpi4(impi)*(2.d0*z*q1+(1.d0+z2)*q3)+q22*q3*(z2*q12+z*(z2-1.d0)*q1*q3-q32) &
       +2.d0*mpi2(impi)*(z*(z2-2.d0)*q13-(1.d0+2.d0*z2)*q12*q3+3.d0*z*(z2-2.d0)*q1*q32+(2.d0*z2*z2-3.d0)*q33))
       
end function R1_3nf

function R2_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R2_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  r2_3nf = coeff*Aq2*q22*(-2.d0*mpi2(impi)*((1.d0+z2)*q1+2.d0*z*q3)&
       +z*q3*((1.d0+z2)*q12-2.d0*q32))/((z2-1.d0)**2.d0*q13*q32) &
!
       +(coeff*2.d0)*Aq3*(mpi2(impi)*(2.d0*z*q12+(1.d0+3.d0*z2)*q1*q3+2.d0*z*q32) &
       +z*q3*(-z*q13-z2*q12*q3+z*q1*q32+q33))/((z2-1.d0)**2.d0*q13*q3) &
!
       +Aq1*coeff*(2.d0*mpi2(impi)*((1.d0+z2)*q12+z*(3.d0+z2)*q1*q3+(1.d0+z2)*q32)&
       +q3*(-(z+z2*z)*q13+(2.d0-5.d0*z2+z2*z2)*q12*q3+z*(1.d0+z2)*q1*q32+(1.d0*z2)*q33))/((z2-1.d0)**2.d0*q12*q32) &
!
       -Int_3pt*4.d0*coeff/((z2-1.d0)**2.d0*q12*(-4.d0*(-1.d0+z2)*mpi2(impi)+q22)*q3)*(q23*q2*q3*(-2.d0*z2*q12+(1.d0+z2)*q32)-&
       8.d0*(z-1.d0)*(1.d0+z)*mpi4(impi)*(z*(2.d0+z2)*q13+(1.d0+2.d0*z2)**2.d0*q12*q3+z*(2.d0+7.d0*z2)*q1*q32+(1.d0+2.d0*z2)*q33)+&
       2.d0*mpi2(impi)*q22*(2.d0*z*q13+(1.d0-z2+6.d0*z2*z2)*q1*q32+(3.d0+3.d0*z2-4.d0*z2*z2)*q33)) &
!
       +mpi(impi)*coeff*(2.d0*mpi2(impi)+q32)*(q22*q3+4.d0*mpi2(impi)*(z*q1+q3))/(q12*(4.d0*(z2-1.d0)*mpi2(impi)-q22)*(4.d0*mpi2(impi)*q3+q33))
       
end function R2_3nf

function R3_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R3_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  R3_3nf = &
       -z*Aq2*coeff*q22*(-4.d0*mpi2(impi)*(q1+z*q3)+q3*(2.d0*z*q12+(z2-1.d0)*q1*q3-2.d0*z*q32))/((z2-1.d0)**2.d0*q12*q33) &
       -z*Aq3*coeff*(mpi2(impi)*(-2.d0*(-3.d0+z2)*q12+4.d0*(1.d0+z2)*q1*q3+4.d0*z*q32)+q3*(-(z2+1.d0)*q13- &
       2.d0*z2*z*q12*q3+(1.d0+z2)*q1*q32+2.d0*z*q33))/(q12*q32*(z2-1.d0)**2.d0) &
!
       -z*Aq1*coeff*(2.d0*mpi2(impi)*(2.d0*q12+4.d0*q1*q3+(1.d0+z2)*q32)+q3*(-2.d0*z*q13+(1.d0-3.d0*z2)*q12*q3+2.d0*z*q1*q32+(1.d0*z2)*q33))/(q1*q33*(z2-1.d0)**2.d0)&
!
       -Int_3pt*4.d0*coeff*z/(q1*(z2-1.d0)**2.d0*(-4.d0*(z2-1.d0)*mpi2(impi)+q22)*q32)*(q23*q2*q3*((1.d0+z2)*q12+z*(z2-1.d0)*q1*q3-(1.d0+z2)*q32)+&
       8.d0*(z-1.d0)*(1.d0+z)*mpi4(impi)*(3.d0*z*q13+(10.d0*z2-1.d0)*q12*q3+3.d0*z*(1.d0+2.d0*z)*q1*q32+(1.d0+2.d0*z2)*q33)+&
       2.d0*mpi2(impi)*q22*(z*(-3.d0+z2)*q13+(3.d0-9.d0*z2)*q12*q3-z*(5.d0+z2)*q1*q32+(-3.d0-3.d0*z2+4.d0*z2*z2)*q33))+&
!
       z*coeff*mpi(impi)*(2.d0*mpi2(impi)+q32)*(q22*q3+4.d0*mpi2(impi)*(z*q1+q3))/(q1*(-4.d0*(z2-1.d0)*mpi2(impi)+q22)*q32*(4.d0*mpi2(impi)+q32))

end function R3_3nf

function R4_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R4_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  R4_3nf = &
       Aq2*coeff*q22*(-2.d0*z2*q12*q3+(1.d0+z2)*q33+2.d0*mpi2(impi)*(2.d0*z*q1+(1.d0+z2)*q3))/(q12*q33*(z2-1.d0)**2.d0) &
!
       +Aq1*coeff*(-2.d0*mpi2(impi)*(2.d0*z*q12+(1.d0+3.d0*z2)*q1*q3+2.d0*z*q32)+q3*(2.d0*z2*q13+2.d0*z2*z*q12*q3 &
       +(1.d0-4.d0*z2+z2*z2)*q1*q32-2.d0*z*q33))/(q1*q33*(z2-1.d0)**2.d0) &
!
       -Aq3*coeff*(2.d0*mpi2(impi)*(-z2*(-3.d0+z2)*q12+z*(3.d0+z2)*q1*q3+(1.d0+z2)*q32)+&
       q3*(-z*(1.d0+z2)*q13-(1.d0-z2+2.d0*z2*z2)*q12*q3+z*(1.d0+z2)*q1*q32+(1.d0+z2)*q33))&
!
       -Int_3pt*4.d0*coeff/(q1*q32*(-4.d0*(z2-1.d0)*mpi2(impi)+q22)*(z2-1.d0)**2.d0)*(q22*q22*q3*((1.d0*z2)*q12*z+(z2-1.d0)**2.d0*q1*q3-2.d0*z*q32)+&
       8.d0*(z2-1.d0)*mpi4(impi)*(3.d0*z2*q13+9.d0*z2*z*q12*q3+(-2.d0+9.d0*z2+2.d0*z2*z2)*q1*q32+z*(2.d0+z2)*q33)+&
       2.d0*mpi2(impi)*q22*(z2*(z2-3.d0)*q13+(2.d0*z-8.d0*z2*z)*q12*q3+(4.d0+5.d0*z2*(z2-3.d0))*q1*q32+2.d0*z*(-3.d0+z2+z2*z2)*q33)) &
!
       +z*coeff*mpi(impi)*(2.d0*mpi2(impi)+q32)*(q22*q3+4.d0*mpi2(impi)*(z*q1+q3))/(q1*q32*q32*(z2-1.d0)**2.d0)

end function R4_3nf

function R5_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R5_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  q2 = q22**(1.d0/2.d0)
  ...
  R5_3nf = Aq2*coeff*q22*(-4.d0*mpion*(q1+z*q3)+q3*(2.0*z*q12+(-1.0+z2)*q1*q3-2.0*z*q32))/(q1*q32*q32*(-1.0+z2)**2) &
       !
       -Aq3*coeff/(q1*q33*(-1.0+z2)**2)*(2.0*mpion*(z*(z2-3.0)*q12-2.0*(1.0+z2)*q1*q3-2.0*z*q32)+q3*((1.0+z2)*q13+&
       2.0*z**3*q12*q3-(1.0+z2)*q1*q32-2.0*z*q33))&
       !
       +Aq1*coeff*(2.0*mpion*(2.0*q12+4.0*z*q1*q3+(1.0+z2)*q32)+q3*(-2.0*z*q13+(1.0-3.0*z2)*q12*q3+2.0*z*q1*q32+(1.0+z2)*q33))/(q32*q32*(z2-1.0)**2.0)&
       !
       +Int_3pt*4.0*coeff/((z2-1.0)**2*(-4.0*(z2-1.0)*mpion*q22)*q33)*(q22**2*q3*((1.0+z2)*q12+z*(z2-1.0)*q1*q3-(1.0+z2)*q32)+&
       8.0*(z-1.0)*(1.0+z)*mpion4*(3.0*z*q13+(10.0*z2-1.0)*q12*q3+3.0*z*(1.0+2.0*z2)*q1*q32+(1.0+2.0*z2)*q33)+ &
       2.0*mpion*q22*(z*(z2-3.0)*q13+(3.0-9.0*z2)*q12*q3-z*(5.0+z2)*q1*q32+(4.0*z2*z2-3.0*z2-3.0)*q33)) &
       !
       -coeff*mpion*(2.0*mpion+q32)*(q22*q3+4.0*mpion*(z*q1+q3))/((-4.0*(z2-1.0)*mpion+q22)*q33*(4.0*mpion+q32))
       
end Function R5_3nf

function R6_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R6_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  R6_3nf = &
       Aq2*coeff*(2.0*mpion2+q22)&
       !
       +Aq1*coeff/q1*(2.0*z*(mpion2+q12)*q3+q1*(8.0*mpion2+3.0*q12+q32))&
       !
       +Aq3*coeff/q3*(2.0*z*q1*(mpion2+q32)+q3*(8.0*mpion2+q12+3.0*q32))-&
       !
       coeff/(q1*(4.d0*mpion2+q12)*(4.d0*(z2-1.d0)*mpion2-q22)*q3*(4.0*mpion2+q32))*&
       ((5.0+z2)*13*q22*q33+8.0*mpion4*mpion2*(z*(4.0*2-3.0)*q12+2.0*(19.0-18.0*z2)*q1*q3+z*(4.0*z2-3.0)*q32)+&
       2.0*mpion4*(4.0*z*(z2-1.0)*q12**2+(77.0-36.0*z2)*q13*q3+2.0*z*(33.0+8.0*z2)*q12*q32+&
       (77.0-36.0*z2)*q1*q33+4.0*z*(z2-1.0)*q32*q32)+2.0*mpion2*q1*q3*((1.0+z2)*q12**2+2.0*z*(9.0+2.0*z2)*q13*q3+(29.0-7.0*z2)*q12*q32+&
       2.0*z*(9.0+2.0*z2)*q1*q33+(10.0+z2)*q32**2)) &
       !
       -Int_3pt*coeff*4.0*(2.0*mpion2+q22)/(q1*(-4.0*(z2-1.0)*mpion2+q22)*q3)*&
...
function R7_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R7_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  coeff = 3.d0*gA4*gA2/(256.d0*fpi4*fpi2)
  
  R7_3nf = &
       coeff*mpion*(2.0*mpion2+q22)/(q12*(-4.0*(z2-1.0)*mpion2+q22))-&
       !
       coeff*Aq3*(2.0*mpion2+q22)*((1.0+z2)*q1+2.0*z*q3)/(q13*(z2-1.0)**2)-&
       !
       coeff*Aq1*(2.0*mpion2+q22)*(2.0*z*q1+(1.0+z2)*q3)/(q12*q3*(-1.0+z2)**2)+&
       !
       coeff*Aq2*(2.0*mpion2+q22)*(2.0*z*q12+(1.0+3.0*z2)*q1*q3+2.0*z*q32)/(q13*q3*(z2-1.0)**2)+&
       !
       4.0*coeff*Int_3pt*(2.0*mpion2+q22)/(q12*(z2-1.0)**2*(4.0*(z2-1.0)*mpion2-q22))*&
       (-q22*((1.0+z2)*q12+z*(3.0+z2)*q1*q3+(1.0+z2)*q32)+&
       4.0*(z2-1.0)*mpion2*((1.0+2.0*z2)*q12+2.0*z*(2.0+z2)*q1*q3+(1.0+2.0*z2)*q32))
       
  
end function R7_3nf 

function R8_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R8_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  R8_3nf = &
       -coeff*z*mpion*(2.0*mpion2+q22)/(q1*(-4.0*(z2-1.0)*mpion2+q22)*q3) + &
       !
       coeff*z*Aq3*(2.0*mpion2+q22)*((1.0+z2)*q1+2.0*z*q3)/(q12*q3*(z2-1.0)**2) + &
       !
       coeff*z*Aq1*(2.0*mpion2+q22)*(2.0*z*q1+(1.0+z2)*q3)/(q1*q32*(z2-1.0)**2) - &
       !
       coeff*z*Aq2*(2.0*mpion2+q22)*(2.0*z*q12+(1.0+3.0*z2)*q1*q3+2.0*z*q32)/(q12*q32*(z2-1.0)**2) - &
       !
       4.0*coeff*Int_3pt*z*(2.0*mpion2+q22)/(q1*(z2-1.0)**2*(4.0*(z2-1.0)*mpion2-q22)*q3)*&
       (-q22*((1.0+z2)*q12+z*(3.0+z2)*q1*q3+(1.0+z2)*q32)+&
       4.0*(z2-1.0)*mpion2*((1.0+2.0*z2)*q12+2.0*z*(2.0+z2)*q1*q3+(1.0+2.0*z2)*q32))
  
end function R8_3nf

function R9_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R9_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  R9_3nf = &
       -coeff/(q12*q32*(z2-1.0)**2)*Aq2*(2.0*mpion2+q22)*((1.0+z2)*q12+z*(3.0+z2)*q1*q3+(1.0+z2)*q32)+ &
       !
       coeff/(q1*q32*(z2-1.0)**2)*Aq1*((1.0+z2)*q13+2.0*z*(2.0+z2)*q12*q3-z2*(z2-7.0)*q1*q32+2.0*z*q33+2.0*mpion2*((1.0+z2)*q1+2.0*z*q3))+&
       !
       4.0*coeff*Int_3pt*z*(2.0*mpion2+q22)/(q1*(-4.0*(z2-1.0)*mpion2+q22)*q3)*(q22*(-2.0*q12+z*(z2-5.0)*q1*q3-2.0*q32)+&
       4.0*(z2-1.0)*mpion2*((2.0+z2)*q12+6.0*z*q1*q3+(2.0+z2)*q32)) - &
       !
       coeff*mpion*(2.0*mpion2+q22)/(q1*(-4.0*(z2-1.0)*mpion2+q22)*q3)
  
  
end function R9_3nf

function R10_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R10_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  R10_3nf = &
       coeff*(z2-1.0)*mpion*(2.0*mpion2+q22)/(-4.0*(z2-1.0)*mpion2+q22) + &
       !
       coeff*Aq2*(2.0*mpion2+q22)*(z*q1+q3)*(q1+z*q3)/(q1*q3*(z2-1.0)) - &
       !
       coeff*Aq1*(z*q13+(1.0+2.0*z2)*q12*q3-z*(z2-4.0)*q1*q32+q33+2.0*mpion2*(z*q1+q3))/(q3*(z2-1.0)) - &
       !
       coeff*Aq3*(q13-z*(z2-4.0)*q12*q3+(1.0+2.0*z2)*q1*q32+z*q33+2.0*mpion2*(q1+z*q3))/((z2-1.0)*q1) + &
       !
       4.0*coeff*Int_3pt*(2.0*mpion2+q22)/((z2-1.0)*(4.0*(z2-1.0)*mpion2-q22))*(-q22*(q12-z*(z2-3.0)*q1*q3+q32)+ &
       4.0*(z2-1.0)*mpion2*((1.0+z2)*q12+4.0*z*q1*q3+(1.0+z2)*q32))
  
end function R10_3nf

function R11_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: R11_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  coeff = gA4*gA2/(256.d0*fpi4*fpi2)

  R11_3nf = &
       -coeff*Aq2*q22*(4.0*mpion2+q12+q32)/((z2-1.0)*q12*q32)+&
       !
       coeff*Aq3*(2.0*mpion2*((z2-1.0)*q12+2.0*z*q1*q3+2.0*q32)+q3*(z*q13+(2.0*z2-1.0)*q12*q3+z*q1*q32+q33))/(q12*q32*(z2-1.0)) + &
       !
       coeff*Aq1*(2.0*mpion2*(2.0*q12+2.0*z*q1*q3+(z2-1.0)*q32)+q1*(q13+z*q12*q3+(2.0*z2-1.0)*q1*q32+z*q33))/(q12*q32*(z2-1.0)) - &
       !
       4.0*coeff*Int_3pt*q22/((z2-1.0)*q12*(-4.0*(z2-1.0)*mpion2+q22)*q32)*(-(2.0*mpion2+q12)*(2.0*mpion2+q32)*(4.0*mpion2+q12+q32)+&
       ...
       !
       coeff*mpion/(q12*(4.0*mpion2+q12)*(-4.0*(z2-1.0)*mpion2+q22)*q32*(4.0*mpion2+q32))*(-2.0*z2*(4.0*mpion4*q12*(4.0*mpion2+q12)+&
       4.0*mpion2*(2.0*mpion2+q12)**2*q32+(2.0*mpion2+q12)**2*q32*q32)+2.0*mpion2*(4.0*mpion2+q12+q32)*(q12*q32+2.0*mpion2*(q12+q32))-&
...
function S1_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: S1_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  coeff = gA4/(128.d0*fpi4*fpi2)
  
  S1_3nf = &
       -Aq1*coeff*(2.0*mpion2+q12) - Aq2*coeff*(4.0*mpion2+q12+z*q1*q3+q32) - Aq3*(2.0*mpion2+q32) + &
       !
       4.0*coeff*Int_3pt*(2.0*mpion2+q12)*(2.0*mpion2+q32) - 2.0*coeff*mpion
  
end function S1_3nf

function S2_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: S2_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  S2_3nf = &
       -Aq1*coeff*((1.0+z2)*q1+2.0*z*q3)/((z2-1.0)**2*q1) - &
       !
       Aq3*coeff*q3*(2.0*z*q1+(1.0+z2)*q3)/((z2-1.0)**2*q12) + &
       !
       Int_3pt*4.0*coeff*q3*(-4.0*z*(z2-1.0)*mpion2+2.0*z*q12+(1.0+3.0*z2)*q1*q3+2.0*z*q32)/(q1*(z2-1.0)**2) + &
       !
       Aq2*coeff*((1.0+z2)*q12+z*(3.0+z2)*q1*q3+(1.0+z2)*q32)/(q12*(z2-1.0)**2)
       
       
end function S2_3nf

function S3_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: S3_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  S3_3nf = &
       Aq3*coeff*((1.0+z2)*q1+2.0*z*q3)/(q1*(z2-1.0)**2) +&
       !
       Aq1*coeff*(2.0*z*q1+(1.0+z2)*q3)/(q3*(z2-1.0)**2) + &
       !
       z*Aq2*coeff*(-2.0*q12+z*(z2-5.0)*q1*q3-2.0*q32)/(q1*q3*(z2-1.0)**2)-&
       !
       Int_3pt*4.0*coeff*(-4.0*(z2-1.0)*mpion2+(1.0+z2)*q12+z*(3.0+z2)*q1*q3+(1.0+z2)*q32)/((z2-1.0)**2)
  
  

end function S3_3nf

function S4_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: S4_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  S4_3nf = &
       z*Aq1*coeff*((1.0+z2)*q1+2.0*z*q3)/(q3*(z2-1.0)**2) + z*Aq3*coeff*(2.0*z*q1+(1.0+z2)*q3)/(q1*(z2-1.0)**2) + &
       !
       Int_3pt*z*4.0*coeff*(4.0*z*(z2-1.0)*mpion2-2.0*z*q12-(1.0+3.0*z2)*q1*q3-2.0*z*q32)/((z2-1.0)**2) - &
       !
       z*Aq2*coeff*((1.0+z2)*q12+z*(3.0+z2)*q1*q3+(1.0+z2)*q32)/(q1*q3*(z2-1.0)**2)
  
    
end function S4_3nf


function S5_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: S5_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  S5_3nf = &
       -Aq1*coeff*q1*((1.0+z2)*q1+2.0*z*q3)/(q32*(z2-1.0)**2) - Aq3*coeff*(2.0*z*q1+(1.0+z2)*q3)/(q3*(z2-1.0)**2) + &
       !
       Int_3pt*4.0*coeff*q1*(-4.0*z*(z2-1.0)*mpion2+2.0*z*q12+(1.0+3.0*z2)*q1*q3+2.0*z*q32)/(q3*(z2-1.0)**2) + &
       !
       Aq2*coeff*((1.0+z2)*q12+z*(3.0+z2)*q1*q3+(1.0+z2)*q32)/(q32*(z2-1.0)**2)

end function S5_3nf 

function S6_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: S6_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  S6_3nf = &
       -Aq3*coeff*q3*(z*q1+q3)/((z2-1.0)**2) + Aq2*coeff*(q12-z*(z2-3.0)*q1*q3+q32)/(z2-1.0) - Aq1*coeff*q1*(q1+z*q3)/(z2-1.0) + &
       !
...
function S7_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)
  
  implicit none 
  real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
  complex*16 :: S7_3nf, int_3pt
  real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3
  integer :: impi
  z2 = z*z
  q1 = q12**(1.d0/2.d0)
  ...
  coeff = gA4/(256.d0*fpi4*fpi2)
  
  S7_3nf = &
       Aq1*coeff*(2.0*mpion2+q32)/((z2-1.0)*q32) - Aq2*coeff*(z*q32*(z*q1+q3)+2.0*mpion2*(q1+z*q3))/(q1*q32*(z2-1.0)) + &
       !
...
function rel2PE_3nf_corr(p,q,r,s,t,u) RESULT(res)
  ...
  REAL*8 :: qtrans1(3), qtrans2(3), qtrans3(3), kmean1(3), kmean2(3),kmean3(3)
  REAL*8 ::  q12, q22, q32, q1xkmean2(3)
  real*8, parameter :: beta_8 = 0.25d0, beta_9 = 0.d0
  REAL*8 :: delta, nucleon_mass, coeff
...
subroutine relcorr_3NF_coeff(p,q,r,s,t,u,rel2PE,rel1PE)
  ...
  complex*16,intent(out) :: rel2PE, rel1PE
  complex*16 :: A123,b123,f123,g123
  complex*16 :: a_temp, b_temp, c_temp, d_temp
  complex*16,parameter :: imag = cmplx(0.d0,-1.d0)
  INTEGER :: p,q,r,s,t,u, m1,m2,m3,m4,m5,m6, spin, iph, t1,t2,t3,t4,t5,t6, Tiso, impi,imnuc
  REAL*8 :: k1(3), k2(3), k3(3), k4(3), k5(3), k6(3), q1xq2(3), q2xq3(3), q1xq3(3)
  REAL*8 :: qtrans1(3), qtrans2(3), qtrans3(3), kmean1(3), kmean2(3),kmean3(3)
  REAL*8 ::  q12, q22, q32, q1xkmean2(3)
  real*8, parameter :: beta_8 = 0.25d0, beta_9 = -0.25d0, Ct_bar = 0.d0, Cs_bar = 0.25d-4
  REAL*8 :: delta, nucleon_mass, coeff
    ...
    a_temp = cmplx(0.d0); b_temp = cmplx(0.d0);
    c_temp = cmplx(0.d0); d_temp = cmplx(0.d0);
    A123 = cmplx(0.d0);  B123 = cmplx(0.d0);
    f123 = cmplx(0.d0);  g123 = cmplx(0.d0);
...
!! 2PE functions
    ...
    a_temp = -gA2/( q12+mpi2(impi) )*( &
         (1.d0-2.d0*beta_8)*(dot_product(qtrans1,qtrans3))**2.d0*delta_tab(m2,m5)*delta_tab(t2,t5) &
         -2.d0*imag*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*delta_tab(t2,t5)*&
         ( (1.d0-2.d0*beta_8)*dot_product(qtrans1,kmean2)+(1.d0+2.d0*beta_8)*dot_product(qtrans1,kmean1))) &
         -gA2*(2.d0*beta_9-1.d0)*(q12*delta_tab(m2,m5)*delta_tab(t2,t5)+&
         2.d0*imag*chp_sigma_dot_q_mtx(m2,m5,q1xkmean2)*delta_tab(t2,t5) )
    
    c_temp = 2.d0*imag*gA2*(2.d0*beta_9+1.d0)*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*delta_tab(t2,t5)
    
    A123 = chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*a_temp &
         + chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,kmean3)*c_temp
    
    
    b_temp = -gA2/(q12+mpi2(impi))*(&
         (1.d0-2.d0*beta_8)*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*dot_product(qtrans1,qtrans3) &
         +2.d0*imag*dot_product(qtrans1,qtrans3)*((1.d0-2.d0*beta_8)*dot_product(qtrans1,kmean2) &
         +(1.d0+2.d0*beta_8)*dot_product(qtrans1,kmean1))*delta_tab(m2,m5)) &
         +2.d0*imag*dot_product(qtrans3,(kmean3-kmean2))*delta_tab(m2,m5) &
         +2.d0*imag*gA2*(2.d0*beta_9-1.d0)*dot_product(qtrans1,kmean2)*delta_tab(m2,m5) &
         -chp_sigma_dot_q_mtx(m2,m5,q1xq3)
    
    d_temp = -2.d0*imag*gA2*(2.d0*beta_9+1.d0)*dot_product(qtrans1,qtrans3)*delta_tab(m2,m5)
    
    B123 = chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*b_temp &
         + chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,kmean3)*d_temp
    
    rel2PE = cmplx( 0.d0,0.d0 )
    
!!$    rel2PE = rel2PE-(gA4/(32.d0*mnuc(imnuc)*fpi4))*chp_sigma_dot_q_mtx(m1,m4,qtrans1) &
!!$         *chp_sigma_dot_q_mtx(m3,m6,qtrans3)/(((q12+mpi2(impi))**2)*(q32+mpi2(impi))) &
!!$         *( (1.d0-2.d0*beta_8)*( tau_dot_tau_tab(t1,t3,t4,t6)*sum(qtrans1*qtrans3)**2 &
!!$         *delta(m2,m5)*delta(t2,t5)&
!!$         +chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*sum(qtrans1*qtrans2) ) &
!!$         -2.d0*imag*( tau_dot_tau_tab(t1,t3,t4,t6)*chp_sigma_dot_q_mtx(m2,m5,q1xq3)&
!!$         *delta(m2,m5)*delta(t2,t5)-chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*sum(qtrans1*qtrans3)*delta(m2,m5) ) &
!!$         *( (1.d0-2.d0*beta_8)*sum(qtrans1*kmean2)+(1.d0+2.d0*beta_8)*sum(qtrans1*kmean1) ) ) &
!!$!         
!!$         +imag*gA2/(32.d0*mnuc(imnuc)*fpi4)*chp_sigma_dot_q_mtx(m1,m4,qtrans1) &
!!$         *chp_sigma_dot_q_mtx(m3,m6,qtrans3)/( (q12+mpi2(impi))*(q32+mpi2(impi)) ) &
!!$         *chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*( sum(qtrans3*kmean3)-sum(qtrans1*kmean1) )*delta(m2,m5)&
!!$!
!!$         -(gA4/(32.d0*mnuc(imnuc)*fpi4) )*chp_sigma_dot_q_mtx(m1,m4,qtrans1)/( (q12+mpi2(impi))*(q32+mpi2(impi)) )&
!!$         *( tau_dot_tau_tab(t1,t3,t4,t6)*delta(t2,t5)*( (2.d0*beta_9-1.d0)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)&
!!$         *(q12*delta(m2,m5)+2.d0*imag*chp_sigma_dot_q_mtx(m2,m5,q1xkmean2) ) &
!!$         -2.d0*imag*(2.d0*beta_9+1.d0)*chp_sigma_dot_q_mtx(m3,m6,kmean3)*chp_sigma_dot_q_mtx(m2,m5,q1xq3))&
!!$         +2.d0*imag*chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*(&
!!$         -(2.d0*beta_9-1.d0)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*sum(qtrans1*kmean2)&
!!$         +(2.d0*beta_9+1.d0)*chp_sigma_dot_q_mtx(m3,m6,kmean3)*sum(qtrans1*qtrans3) )*delta(m2,m5) )
!

!!$    qtrans1 = (/1,1,1/) 
!!$    print*, qtrans1
!!$    print*, "sigmaXsigma_dot_q = ",sigmaXsigma_dot_q(1,1,-1,-1,qtrans1)
!!$    STOP
    
    
! 1PE functions
    f123 = 1.d0/(q12+mpi2(impi))*(&
         (1.-2.*beta_8)*dot_product(qtrans1,qtrans3)&
         *( Cs_bar*chp_sigma_dot_q_mtx(m2,m5,qtrans1)*delta(m3,m6) &
         +Ct_bar*chp_sigma_dot_q_mtx(m3,m6,qtrans1)*delta(m2,m5) ) &
!
         +2.*imag*Ct_bar*sigmaXsigma_dot_q(m2,m3,m5,m6,qtrans1)*(&
         (1.-2.*beta_8)*dot_product(qtrans1,kmean2)+(1.d0+2.d0*beta_8)*dot_product(qtrans1,kmean1) )&
         ) &
!
         +(2.d0*beta_9 - 1.d0)*( Cs_bar*chp_sigma_dot_q_mtx(m2,m5,qtrans3)*delta(m3,m6) &
         +Ct_bar*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*delta(m2,m5) ) &
!
         +2.d0*imag*Ct_bar*(2.d0*beta_9 - 1.d0)*sigmaXsigma_dot_q(m2,m3,m5,m6,kmean2) 
    
    g123 = -2.d0*imag*Ct_bar*(2.d0*beta_9 + 1.d0)*sigmaXsigma_dot_q(m2,m3,m5,m6,qtrans1)
    
    
    rel1PE = cmplx(0.d0)
!123

    rel2PE = gA2/((32.d0*mnuc(imnuc)*fpi4)*(q12+mpi2(impi))*(q32+mpi2(impi))) &
         *(tau_dot_tau_tab(t1,t3,t4,t6)*A123+chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*B123)
    
    rel1PE = gA2/( 8.d0*mnuc(imnuc)*fpi2*(q12+mpi2(impi)) )*chp_tau_dot_tau_mtx(t1,t2,t4,t5)*delta(t3,t6)&
         *( chp_sigma_dot_q_mtx(m1,m4,qtrans1)*f123 + chp_sigma_dot_q_mtx(m1,m4,kmean1)*g123)
    
    
  end subroutine relcorr_3NF_coeff


function relcorr_3NF(p,q,r,s,t,u)
  ...
  complex*16 :: relcorr_3NF
  complex*16 :: A123,b123,f123,g123
  complex*16 :: a_temp, b_temp, c_temp, d_temp, imag
  INTEGER :: p,q,r,s,t,u, m1,m2,m3,m4,m5,m6, spin, iph, t1,t2,t3,t4,t5,t6, Tiso, impi,imnuc
  REAL*8 :: k1(3), k2(3), k3(3), k4(3), k5(3), k6(3), q1xq2(3), q2xq3(3), q1xq3(3)
  REAL*8 :: qtrans1(3), qtrans2(3), qtrans3(3), kmean1(3), kmean2(3),kmean3(3)
  REAL*8 ::  q12, q22, q32, q1xkmean2(3)
  real*8, parameter :: beta_8 = 0.25d0, beta_9 = -0.25d0, Ct_bar = 0.25d-4, Cs_bar = 0.d0
  REAL*8 :: delta, nucleon_mass, coeff
...
!! 2PE functions
    ...
    a_temp = -gA2/(q12+mpi2(impi))*( &
         (1.d0-2.d0*beta_8)*(sum(qtrans1*qtrans3))**2.d0*delta_tab(m2,m5)*delta_tab(t2,t5) &
         -2.d0*imag*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*delta_tab(t2,t5)*&
         ( (1.d0-2.d0*beta_8)*sum(qtrans1*kmean2)+(1.d0+2.d0*beta_8)*sum(qtrans1*kmean1))) &
         -gA2*(2.d0*beta_9-1.d0)*(q12*delta_tab(m2,m5)*delta_tab(t2,t5)+&
         2.d0*imag*chp_sigma_dot_q_mtx(m2,m5,q1xkmean2)*delta_tab(t2,t5) )
    ...
    b_temp = -gA2/(q12+mpi2(impi))*(&
         (1.d0-2.d0*beta_8)*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*sum(qtrans1*qtrans3) &
         +2.d0*imag*sum(qtrans1*qtrans3)*((1.d0-2.d0*beta_8)*sum(qtrans1*kmean2) &
         +(1.d0+2.d0*beta_8)*sum(qtrans1*kmean1))*delta_tab(m2,m5)) &
         +2.d0*imag*sum(qtrans3*(kmean3-kmean2))*delta_tab(m2,m5) &
         +2.d0*imag*gA2*(2.d0*beta_9-1.d0)*sum(qtrans1*kmean2)*delta_tab(m2,m5) &
         -chp_sigma_dot_q_mtx(m2,m5,q1xq3)
    
    d_temp = -2.d0*imag*gA2*(2.d0*beta_9+1.d0)*sum(qtrans1*qtrans3)*delta_tab(m2,m5)
    
    B123 = chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*b_temp &
         + chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,kmean3)*d_temp

    relcorr_3NF = gA2/((32.d0*mnuc(imnuc)*fpi4)*(q12+mpi2(impi))*(q32+mpi2(impi))) &
         *(tau_dot_tau_tab(t1,t3,t4,t6)*A123+chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*B123)

    
  end function relcorr_3NF
...
!!$    if ( nz1 + nz2 /= nz3 + nz4 ) return 
  ...
  complex*16 function chiral_3nf(p,q,r,s,t,u)
    ...
    REAL*8 :: qtrans1(3), qtrans2(3), qtrans3(3), prel(3), pprel(3),kmean1(3),kmean2(3),kmean3(3)
    REAL*8 :: q22, q32, q12, freg12, freg22, freg32, Aq1, Aq2, Aq3
    REAL*8 :: delta, nucleon_mass, coeff
    real*8 :: z13,z12,z23
    real*8 :: c1_bar, c2_bar, c3_bar,c4_bar
    complex*16 :: VNNLO, vcd, vce, v2pe, v3nf_sum, v3nf_sum1, v3nf_sum2, v3nf_sum3
    complex*16 :: vn3lo_2PE, vn3lo_2P1PE, vn3lo_sum1, vn3lo_sum2, vn3lo_sum3
...
!
!   N3LO 3NF Contributions (Dimensional Regularization)
!   See references [ref1] = [Bernard, et al] PHYSICAL REVIEW C 77, 064004 (2008)
!


!   
! N3LO 2PE Contributions 
!


!
!1st contribution is absorbed into shifting of constants used above
!
    impi = 2
...
!!$    c4_bar = c4_3nf - gA4*mpi(impi)/(16.d0*pi*fpi2)
    ...
    vn3lo_sum1 = cmplx(0.d0)
    vn3lo_sum2 = cmplx(0.d0)
    vn3lo_sum3 = cmplx(0.d0)
...
!!$    vn3lo_sum1 = & 
!!$         sigma_dot_q_ope141*sigma_dot_q_ope252 *const(2)/(2.d0*fpi2)*( &
!!$         (-c1_bar*4.d0*mpi2(impi) +c3_bar*2.d0 * sum(qtrans1*qtrans2) )*tau_dot_tau_tab(t1,t2,t4,t5) &
!!$         *delta_tab(m3,m6)*delta_tab(t3,t6) &
!!$         +  c4_bar * chp_sigma_dot_q_mtx(m3,m6,q1xq2) *( chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6) &
!!$         - chp_tau1Xtau2_dot_tau3(t2,t1,t3,t5,t4,t6)) )
!!$ 
!!$    vn3lo_sum2 = &
!!$         sigma_dot_q_ope141*sigma_dot_q_ope363 *const(2)/(2.d0*fpi2)*( &
!!$         (-c1_bar*4.d0*mpi2(2) +c3_bar*2.d0 * sum(qtrans1*qtrans3) )*tau_dot_tau_tab(t1,t3,t4,t6) &
!!$         *delta_tab(m2,m5)*delta_tab(t2,t5) &
!!$         +  c4_bar * chp_sigma_dot_q_mtx(m2,m5,q1xq3) * (chp_tau1Xtau2_dot_tau3(t1,t3,t2,t4,t6,t5) &
!!$         - chp_tau1Xtau2_dot_tau3(t3,t1,t2,t6,t4,t5)) )
!!$
!!$    vn3lo_sum3 = &
!!$         sigma_dot_q_ope252*sigma_dot_q_ope363 *const(2)/(2.d0*fpi2)*( &
!!$         (-c1_bar*4.d0*mpi2(2) +c3_bar*2.d0 * sum(qtrans2*qtrans3) )*tau_dot_tau_tab(t2,t3,t5,t6) &
!!$         *delta_tab(m1,m4)*delta_tab(t1,t4) &
!!$         +  c4_bar * chp_sigma_dot_q_mtx(m1,m4,q2xq3) *(chp_tau1Xtau2_dot_tau3(t2,t3,t1,t5,t6,t4) &
!!$         - chp_tau1Xtau2_dot_tau3(t3,t2,t1,t6,t5,t4) ) )
...
!!$    vn3lo_2PE = vn3lo_2PE + &
!!$         sigma_dot_q_ope141*sigma_dot_q_ope252*const(2)/(2.d0*fpi2)*( &
!!$         tau_dot_tau_tab(t1,t2,t4,t5)*(-4.d0*c1_bar*mpi2(impi)+2.d0*c3_bar*sum(qtrans1*qtrans2) ) &
!!$         *delta_tab(m3,m6)*delta_tab(t3,t6) &
!!$         +c4_bar*chp_sigma_dot_q_mtx(m3,m6,q1xq2)*chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6) )
!!$    
!!$!312
!!$    vn3lo_2PE = vn3lo_2PE + &
!!$         sigma_dot_q_ope363*sigma_dot_q_ope252*const(2)/(2.d0*fpi2)*( &
!!$         tau_dot_tau_tab(t3,t2,t6,t5)*(-4.d0*c1_bar*mpi2(impi)+2.d0*c3_bar*sum(qtrans3*qtrans2) ) &
!!$         *delta_tab(m1,m4)*delta_tab(t1,t4) &
!!$         +c4_bar*chp_sigma_dot_q_mtx(m1,m4,-1.d0*q2xq3)*chp_tau1Xtau2_dot_tau3(t3,t2,t1,t6,t5,t4) )
!!$
!!$!321
!!$    vn3lo_2PE = vn3lo_2PE + &
!!$         sigma_dot_q_ope363*sigma_dot_q_ope141*const(2)/(2.d0*fpi2)*( &
!!$         tau_dot_tau_tab(t3,t1,t6,t4)*(-4.d0*c1_bar*mpi2(impi)+2.d0*c3_bar*sum(qtrans3*qtrans1) ) &
!!$         *delta_tab(m2,m5)*delta_tab(t2,t5) &
!!$         +c4_bar*chp_sigma_dot_q_mtx(m2,m5,-1.d0*q1xq3)*chp_tau1Xtau2_dot_tau3(t3,t1,t2,t6,t4,t5) )
!!$
!!$!231    
!!$    vn3lo_2PE = vn3lo_2PE + &
!!$         sigma_dot_q_ope252*sigma_dot_q_ope141*const(2)/(2.d0*fpi2)*( &
!!$         tau_dot_tau_tab(t2,t1,t5,t4)*(-4.d0*c1_bar*mpi2(impi)+2.d0*c3_bar*sum(qtrans2*qtrans1) ) &
!!$         *delta_tab(m3,m6)*delta_tab(t3,t6) &
!!$         +c4_bar*chp_sigma_dot_q_mtx(m3,m6,-1.d0*q1xq2)*chp_tau1Xtau2_dot_tau3(t2,t1,t3,t5,t4,t6) )
!!$
!!$!213
!!$    vn3lo_2PE = vn3lo_2PE + &
!!$         sigma_dot_q_ope252*sigma_dot_q_ope363*const(2)/(2.d0*fpi2)*( &
!!$         tau_dot_tau_tab(t2,t3,t5,t6)*(-4.d0*c1_bar*mpi2(impi)+2.d0*c3_bar*sum(qtrans2*qtrans3) ) &
!!$         *delta_tab(m1,m4)*delta_tab(t1,t4) &
!!$         +c4_bar*chp_sigma_dot_q_mtx(m1,m4,q2xq3)*chp_tau1Xtau2_dot_tau3(t2,t3,t1,t5,t6,t4) )
!!$
!!$    
!!$! coefficient used for graph 20 in [ref1]
!!$!
!!$! gA^4/(256*pi*fpi^6)
!!$    
    Aq1 = chp_DR_2PE_loop_A(Q12,impi)
...
!!$    
!!$!123
!!$    vn3lo_2PE = vn3lo_2PE + coeff*sigma_dot_q_ope141*sigma_dot_q_ope363*&
!!$         (tau_dot_tau_tab(t1,t3,t4,t6)*(mpi(impi)*(mpi2(impi) + 3.d0*Q12 + 3.d0*Q32 + 4.d0*sum(qtrans1*qtrans3)) &
!!$         +(2.d0*mpi2(impi) + Q12 + Q32 + 2.d0*sum(qtrans1*qtrans3) ) &
!!$         *(3.d0*mpi2(impi) + 3.d0*Q12 + 3.d0*Q32 + 4.d0*sum(qtrans1*qtrans3)) * Aq2) &
!!$         *delta_tab(m2,m5)*delta_tab(t2,t5) &
...
!!$!132
!!$    vn3lo_2PE = vn3lo_2PE + &
!!$         coeff*sigma_dot_q_ope141*sigma_dot_q_ope252*&
...
!!$         *delta_tab(m3,m6)*delta_tab(t3,t6) &
!!$         -chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*chp_sigma_dot_q_mtx(m3,m6,q1xq2) &
!!$         *(mpi(impi)+(4.d0*mpi2(impi)+Q12+Q22+2.d0*sum(qtrans1*qtrans2))*Aq3))
!!$    
!!$!312
!!$    vn3lo_2PE = vn3lo_2PE + &
!!$         coeff*sigma_dot_q_ope363*sigma_dot_q_ope252*&
!!$         (tau_dot_tau_tab(t3,t2,t6,t5)*(mpi(impi)*(mpi2(impi) + 3.d0*Q32 + 3.d0*Q22 + 4.d0*sum(qtrans3*qtrans2)) &
!!$         +(2.d0*mpi2(impi) + Q32 + Q22 + 2.d0*sum(qtrans3*qtrans2) ) &
!!$         *(3.d0*mpi2(impi) + 3.d0*Q32 + 3.d0*Q22 + 4.d0*sum(qtrans3*qtrans2)) * Aq1) &
!!$         *delta_tab(m1,m4)*delta_tab(t1,t4) &
!!$         -chp_tau1Xtau2_dot_tau3(t3,t2,t1,t6,t5,t4)*&
!!$         chp_sigma_dot_q_mtx(m1,m4,-1.d0*q2xq3) *(mpi(impi)+(4.d0*mpi2(impi)+Q32+Q22+2.d0*sum(qtrans3*qtrans2))*Aq1))
!!$    
!!$!321
!!$    vn3lo_2PE = vn3lo_2PE + coeff*sigma_dot_q_ope141*sigma_dot_q_ope363*&
!!$         (tau_dot_tau_tab(t1,t3,t4,t6)*(mpi(impi)*(mpi2(impi) + 3.d0*Q12 + 3.d0*Q32 + 4.d0*sum(qtrans1*qtrans3)) &
...
!!$         *(3.d0*mpi2(impi) + 3.d0*Q12 + 3.d0*Q32 + 4.d0*sum(qtrans1*qtrans3)) * Aq2) &
!!$         *delta_tab(m2,m5)*delta_tab(t2,t5) &
!!$         -chp_tau1Xtau2_dot_tau3(t3,t1,t2,t6,t4,t5)*chp_sigma_dot_q_mtx(m2,m5,-1.d0*q1xq3) &
!!$         *(mpi(impi)+(4.d0*mpi2(impi)+Q12+Q32+2.d0*sum(qtrans1*qtrans3))*Aq2))
!!$    
!!$!231
!!$    vn3lo_2PE = vn3lo_2PE + coeff*sigma_dot_q_ope141*sigma_dot_q_ope252*&
!!$         (tau_dot_tau_tab(t1,t2,t4,t5)*(mpi(impi)*(mpi2(impi) + 3.d0*Q12 + 3.d0*Q22 + 4.d0*sum(qtrans1*qtrans2)) &
!!$         +(2.d0*mpi2(impi) + Q12 + Q22 + 2.d0*sum(qtrans1*qtrans2) ) &
!!$         *(3.d0*mpi2(impi) + 3.d0*Q12 + 3.d0*Q22 + 4.d0*sum(qtrans1*qtrans2)) * Aq3) &
!!$         *delta_tab(m3,m6)*delta_tab(t3,t6) &
!!$         -chp_tau1Xtau2_dot_tau3(t2,t1,t3,t5,t4,t6)*chp_sigma_dot_q_mtx(m3,m6,-1.d0*q1xq2) &
!!$         *(mpi(impi)+(4.d0*mpi2(impi)+Q12+Q22+2.d0*sum(qtrans1*qtrans2))*Aq3))
!!$    
!!$!213
!!$    vn3lo_2PE = vn3lo_2PE + coeff*sigma_dot_q_ope252*sigma_dot_q_ope363*&
...
!!$         *delta_tab(m1,m4)*delta_tab(t1,t4) &
!!$         -chp_tau1Xtau2_dot_tau3(t2,t3,t1,t5,t6,t4)*chp_sigma_dot_q_mtx(m1,m4,q2xq3) &
...
!!$    vn3lo_2P1PE = vn3lo_2P1PE + sigma_dot_q_ope252*&
!!$         (tau_dot_tau_tab(t1,t2,t4,t5)*(chp_sigma_dot_q_mtx(m3,m6,qtrans1)*dot_product(qtrans1,qtrans3)*V3NF_F1(Q12,Aq1,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3NF_F2(Q12,Aq1,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3nf_F3(Q12,Aq1,impi))&
!!$         *delta_tab(m1,m4)*delta_tab(t3,t6) &
!!$         +tau_dot_tau_tab(t3,t2,t6,t5)*( (chp_sigma_dot_q_mtx(m1,m4,qtrans1)*dot_product(qtrans1,qtrans2)*V3NF_F4(Aq1) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans2)*V3NF_F5(Q12,Aq1) )*delta(m3,m6)*delta(t1,t4) &
!!$         +(chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3NF_F6(Q12,Aq1,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3NF_F7(Q12,Aq1,impi) )*delta(m1,m4)*delta(t1,t4) ) &
!!$         +(chp_tau1Xtau2_dot_tau3(t1,t3,t2,t4,t6,t5)-chp_tau1Xtau2_dot_tau3(t3,t1,t2,t6,t4,t5) ) &
!!$         *sigmaXsigma_dot_q(m1,m3,m4,m6,qtrans1)*V3NF_F8(Q12,Aq1,impi)) 

!!$    vn3lo_2P1PE = vn3lo_2P1PE + sigma_dot_q_ope141*&
!!$         (tau_dot_tau_tab(t2,t1,t5,t4)*(chp_sigma_dot_q_mtx(m3,m6,qtrans2)*dot_product(qtrans2,qtrans3)*V3NF_F1(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3NF_F2(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3nf_F3(Q22,Aq2,impi))&
!!$         *delta_tab(m2,m5)*delta_tab(t3,t6) &
!!$         +tau_dot_tau_tab(t3,t1,t6,t4)*( (chp_sigma_dot_q_mtx(m1,m4,qtrans2)*dot_product(qtrans2,qtrans1)*V3NF_F4(Aq2) &
!!$         +chp_sigma_dot_q_mtx(m2,m5,qtrans1)*V3NF_F5(Q22,Aq2) )*delta(m3,m6)*delta(t2,t5)&
!!$         +(chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3NF_F6(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3NF_F7(Q22,Aq2,impi) )*delta(m2,m5)*delta(t2,t5) ) &
!!$         +(chp_tau1Xtau2_dot_tau3(t2,t3,t1,t5,t6,t4)-chp_tau1Xtau2_dot_tau3(t3,t2,t1,t6,t5,t6) ) &
!!$         *sigmaXsigma_dot_q(m2,m3,m5,m6,qtrans2)*V3NF_F8(Q22,Aq2,impi) )
...
!! zij = Qi*Qj/(|Qi||Qj|)
    ...
    z13 = dot_product(qtrans1,qtrans3)/(sqrt(q12*q32))

!!$    vn3lo_ring = chp_sigma_dot_sigma_mtx(m1,m2,m4,m5)*tau_dot_tau_tab(t2,t3,t5,t6)*vn3lo_R1(q12,q32,z13,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_sigma_mtx(m2,m5,qtrans1)*tau_dot_tau_tab(t2,t3,t5,t6)*vn3lo_R2(q12,q32,z13,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m2,m5,qtrans3)*tau_dot_tau_tab(t2,t3,t5,t6)*vn3lo_R3(q12,q32,z13,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*chp_sigma_dot_q_mtx(m2,m5,qtrans1)*tau_dot_tau_tab(t2,t3,t5,t6)*vn3lo_R4(q12,q32,z13,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*chp_sigma_dot_q_mtx(m2,m5,qtrans3)*tau_dot_tau_tab(t2,t3,t5,t6)*vn3lo_R5(q12,q32,z13,impi) &
...
!! 1PE-contacts
    !const(14)*8 = gA4/(16*pi*fpi4)
 

    
!! 2PE-contacts see Bernard, et al.
!note:  const(23)  = gA2/(24*pi*fpi4)


!!$!123
!!$!Eq 3.6
!!$    cont_2PE = cont_2PE + (const(23)*gA2/2.d0)*CT(0)*(2.d0*tau_dot_tau_tab(t1,t2,t4,t5)*chp_sigma_dot_sigma_mtx(m2,m3,m5,m6)&
!!$         *(3.d0*mpi(impi)-mpi3(impi)/(4.d0*mpi2(impi)+Q12)+2.d0*(2.d0*mpi2(impi)+Q12)*Aq1) &
!!$         *delta(t3,t6)*delta(m1,m4) &
!!$         +9.d0*(chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m2,m5,qtrans1)&
!!$         -Q12*chp_sigma_dot_sigma_mtx(m1,m2,m4,m5))*Aq1*delta(t3,t6)*delta(t1,t4)*delta(t2,t5)*delta(m3,m6) )&
!!$!Eq 3.10
!!$         - const(23)*CT(0)*tau_dot_tau_tab(t1,t2,t4,t5)*chp_sigma_dot_sigma_mtx(m2,m3,m5,m6) &
!!$         *(mpi(impi)+(2.d0*mpi2(impi)+q12)*Aq1)*delta(m1,m4)*delta(t3,t6)
!!$
!!$
!!$!132
!!$!Eq 3.6
!!$    cont_2PE = cont_2PE + (const(23)*gA2/2.d0)*CT(0)*(2.d0*tau_dot_tau_tab(t1,t3,t4,t6)*chp_sigma_dot_sigma_mtx(m3,m2,m6,m5)&
!!$         *(3.d0*mpi(impi)-mpi3(impi)/(4.d0*mpi2(impi)+Q12)+2.d0*(2.d0*mpi2(impi)+Q12)*Aq1) &
!!$         *delta(t2,t5)*delta(m1,m4) &
!!$         +9.d0*(chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans1)&
!!$         -Q12*chp_sigma_dot_sigma_mtx(m1,m3,m4,m6))*Aq1*delta(t2,t5)*delta(t1,t4)*delta(t3,t6)*delta(m2,m5) )&
!!$!Eq 3.10
!!$         - const(23)*CT(0)*tau_dot_tau_tab(t1,t3,t4,t6)*chp_sigma_dot_sigma_mtx(m3,m2,m6,m5) &
!!$         *(mpi(impi)+(2.d0*mpi2(impi)+q12)*Aq1)*delta(m1,m4)*delta(t2,t5)
!!$
!!$!312
!!$!Eq 3.6
!!$    cont_2PE = cont_2PE + (const(23)*gA2/2.d0)*CT(0)*(2.d0*tau_dot_tau_tab(t3,t1,t6,t4)*chp_sigma_dot_sigma_mtx(m1,m2,m4,m5)&
!!$         *(3.d0*mpi(impi)-mpi3(impi)/(4.d0*mpi2(impi)+Q32)+2.d0*(2.d0*mpi2(impi)+Q32)*Aq3) &
!!$         *delta(t2,t5)*delta(m3,m6) &
!!$         +9.d0*(chp_sigma_dot_q_mtx(m3,m6,qtrans3)*chp_sigma_dot_q_mtx(m1,m4,qtrans3)&
!!$         -Q32*chp_sigma_dot_sigma_mtx(m3,m1,m6,m4))*Aq3*delta(t2,t5)*delta(t3,t6)*delta(t1,t4)*delta(m2,m5) )&
!!$!Eq 3.10
!!$         - const(23)*CT(0)*tau_dot_tau_tab(t3,t1,t6,t4)*chp_sigma_dot_sigma_mtx(m1,m2,m4,m5) &
!!$         *(mpi(impi)+(2.d0*mpi2(impi)+q32)*Aq3)*delta(m3,m6)*delta(t2,t5)
!!$
!!$
!!$!321
!!$!Eq 3.6
!!$    cont_2PE = cont_2PE + (const(23)*gA2/2.d0)*CT(0)*(2.d0*tau_dot_tau_tab(t3,t2,t6,t5)*chp_sigma_dot_sigma_mtx(m2,m1,m5,m4)&
!!$         *(3.d0*mpi(impi)-mpi3(impi)/(4.d0*mpi2(impi)+Q32)+2.d0*(2.d0*mpi2(impi)+Q32)*Aq3) &
!!$         *delta(t1,t4)*delta(m3,m6) &
!!$         +9.d0*(chp_sigma_dot_q_mtx(m3,m6,qtrans3)*chp_sigma_dot_q_mtx(m2,m5,qtrans3)&
!!$         -Q32*chp_sigma_dot_sigma_mtx(m3,m2,m6,m5))*Aq3*delta(t1,t4)*delta(t3,t6)*delta(t2,t5)*delta(m1,m4) )&
!!$!Eq 3.10
!!$         - const(23)*CT(0)*tau_dot_tau_tab(t3,t2,t6,t5)*chp_sigma_dot_sigma_mtx(m2,m1,m5,m4) &
!!$         *(mpi(impi)+(2.d0*mpi2(impi)+q32)*Aq3)*delta(m3,m6)*delta(t1,t4)
!!$
!!$
!!$!231
!!$!Eq 3.6
!!$    cont_2PE = cont_2PE + (const(23)*gA2/2.d0)*CT(0)*(2.d0*tau_dot_tau_tab(t2,t3,t5,t6)*chp_sigma_dot_sigma_mtx(m3,m1,m6,m4)&
!!$         *(3.d0*mpi(impi)-mpi3(impi)/(4.d0*mpi2(impi)+Q22)+2.d0*(2.d0*mpi2(impi)+Q22)*Aq2) &
!!$         *delta(t1,t4)*delta(m2,m5) &
!!$         +9.d0*(chp_sigma_dot_q_mtx(m2,m5,qtrans2)*chp_sigma_dot_q_mtx(m3,m6,qtrans2)&
!!$         -Q22*chp_sigma_dot_sigma_mtx(m2,m3,m5,m6))*Aq2*delta(t1,t4)*delta(t2,t5)*delta(t3,t6)*delta(m1,m4) )&
!!$!Eq 3.10
!!$         - const(23)*CT(0)*tau_dot_tau_tab(t2,t3,t5,t6)*chp_sigma_dot_sigma_mtx(m3,m1,m6,m4) &
!!$         *(mpi(impi)+(2.d0*mpi2(impi)+q22)*Aq2)*delta(m2,m5)*delta(t1,t4)
!!$
!!$
!!$!213
!!$!Eq 3.6
!!$    cont_2PE = cont_2PE + (const(23)*gA2/2.d0)*CT(0)*(2.d0*tau_dot_tau_tab(t2,t1,t5,t4)*chp_sigma_dot_sigma_mtx(m1,m3,m4,m6)&
!!$         *(3.d0*mpi(impi)-mpi3(impi)/(4.d0*mpi2(impi)+Q22)+2.d0*(2.d0*mpi2(impi)+Q22)*Aq2) &
!!$         *delta(t3,t6)*delta(m2,m5) &
!!$         +9.d0*(chp_sigma_dot_q_mtx(m2,m5,qtrans2)*chp_sigma_dot_q_mtx(m1,m4,qtrans2)&
!!$         -Q22*chp_sigma_dot_sigma_mtx(m2,m1,m5,m4))*Aq2*delta(t3,t6)*delta(t2,t5)*delta(t1,t4)*delta(m3,m6) )&
!!$!Eq 3.10
!!$         - const(23)*CT(0)*tau_dot_tau_tab(t2,t1,t5,t4)*chp_sigma_dot_sigma_mtx(m1,m3,m4,m6) &
!!$         *(mpi(impi)+(2.d0*mpi2(impi)+q22)*Aq2)*delta(m2,m5)*delta(t3,t6)
...
!213
    ...
    relcorr_2PE = cmplx(0.d0); relcorr_1PE = cmplx(0.d0);

    imnuc = 0;
   
!!$!123
!!$    call relcorr_3NF_coeff(p,q,r,s,t,u,rel2PE,rel1PE)
!!$    relcorr_2PE = relcorr_2PE + rel2PE
!!$    relcorr_1PE = relcorr_1PE + rel1PE
!!$!132
!!$    call relcorr_3NF_coeff(p,r,q,s,u,t,rel2PE,rel1PE)
!!$    relcorr_2PE = relcorr_2PE + rel2PE
!!$    relcorr_1PE = relcorr_1PE + rel1PE
!!$!312
!!$    call relcorr_3NF_coeff(r,p,q,u,s,t,rel2PE,rel1PE)
!!$    relcorr_2PE = relcorr_2PE + rel2PE
!!$    relcorr_1PE = relcorr_1PE + rel1PE
!!$!321
!!$    call relcorr_3NF_coeff(r,q,p,u,t,s,rel2PE,rel1PE)
!!$    relcorr_2PE = relcorr_2PE + rel2PE
!!$    relcorr_1PE = relcorr_1PE + rel1PE
!!$!231
!!$    call relcorr_3NF_coeff(q,r,p,t,u,s,rel2PE,rel1PE)
!!$    relcorr_2PE = relcorr_2PE + rel2PE
!!$    relcorr_1PE = relcorr_1PE + rel1PE
!!$!213
!!$    call relcorr_3NF_coeff(q,p,r,t,s,u,rel2PE,rel1PE)
!!$    relcorr_2PE = relcorr_2PE + rel2PE
!!$    relcorr_1PE = relcorr_1PE + rel1PE
    ...
    chiral_3nf = freg_3nfnonlocal(k1,k2,k3)*freg_3nfnonlocal(k4,k5,k6)*hbarc**6*(relcorr_2PE + relcorr_1PE)/volume**2
    
    !       chiral_3nf = cmplx(0d0)
    !chiral_3nf = hbarc**6*(relcorr_2PE + relcorr_1PE)/volume**2
    
  end function chiral_3nf

!!$!123
!!$   !Eq 2.1
!!$    cont_1PE = -8.d0*const(14)*CT(0)*mpi(impi)*sigma_dot_q_ope141*chp_sigma_dot_q_mtx(m3,m6,qtrans1) &
!!$         *tau_dot_tau_tab(t1,t2,t4,t5)*delta(m2,m5)*delta(t3,t6) &
!!$    !Eq 2.3
!!$         + CT(0)*(4.d0*const(14))*mpi(impi)*sigma_dot_q_ope141 &
!!$         *(2.d0* tau_dot_tau_tab(t1,t3,t4,t6)*chp_sigma_dot_q_mtx(m3,m6,qtrans1)*delta(t2,t5)*delta(m2,m5) &
!!$         - tau1_dot_tauXtau_tab(t1,t2,t3,t4,t5,t6)*sigmaXsigma_dot_q(m2,m3,m5,m6,qtrans1) )
!!$
!!$!132
!!$   !Eq 2.1
!!$    cont_1PE =cont_1PE - 8.d0*const(14)*CT(0)*mpi(impi)*sigma_dot_q_ope141*chp_sigma_dot_q_mtx(m2,m5,qtrans1) &
!!$         *tau_dot_tau_tab(t1,t3,t4,t6)*delta(m3,m6)*delta(t2,t5) &
!!$    !Eq 2.3
!!$         + CT(0)*(4.d0*const(14))*mpi(impi)*sigma_dot_q_ope141 &
!!$         *(2.d0* tau_dot_tau_tab(t1,t2,t4,t5)*chp_sigma_dot_q_mtx(m2,m5,qtrans1)*delta(t3,t6)*delta(m3,m6) &
!!$         - tau1_dot_tauXtau_tab(t1,t3,t2,t4,t6,t5)*sigmaXsigma_dot_q(m3,m2,m6,m5,qtrans1) )
!!$
!!$!312
!!$   !Eq 2.1
!!$    cont_1PE =cont_1PE - 8.d0*const(14)*CT(0)*mpi(impi)*sigma_dot_q_ope363*chp_sigma_dot_q_mtx(m2,m5,qtrans3) &
!!$         *tau_dot_tau_tab(t3,t1,t6,t4)*delta(m1,m4)*delta(t2,t5) &
!!$    !Eq 2.3
!!$         + CT(0)*(4.d0*const(14))*mpi(impi)*sigma_dot_q_ope363 &
!!$         *(2.d0* tau_dot_tau_tab(t3,t2,t6,t5)*chp_sigma_dot_q_mtx(m2,m5,qtrans3)*delta(t1,t4)*delta(m1,m4) &
!!$         - tau1_dot_tauXtau_tab(t3,t1,t2,t6,t4,t5)*sigmaXsigma_dot_q(m1,m2,m4,m5,qtrans3) )
!!$    
!!$!321
!!$   !Eq 2.1
!!$    cont_1PE =cont_1PE - 8.d0*const(14)*CT(0)*mpi(impi)*sigma_dot_q_ope363*chp_sigma_dot_q_mtx(m1,m4,qtrans3) &
!!$         *tau_dot_tau_tab(t3,t2,t6,t5)*delta(m2,m5)*delta(t1,t4) &
!!$    !Eq 2.3
!!$         + CT(0)*(4.d0*const(14))*mpi(impi)*sigma_dot_q_ope363 &
!!$         *(2.d0* tau_dot_tau_tab(t3,t1,t6,t4)*chp_sigma_dot_q_mtx(m1,m4,qtrans3)*delta(t2,t5)*delta(m2,m5) &
!!$         - tau1_dot_tauXtau_tab(t3,t2,t1,t6,t5,t4)*sigmaXsigma_dot_q(m2,m1,m5,m4,qtrans3) )
!!$
!!$!231
!!$   !Eq 2.1
!!$    cont_1PE =cont_1PE - 8.d0*const(14)*CT(0)*mpi(impi)*sigma_dot_q_ope252*chp_sigma_dot_q_mtx(m1,m4,qtrans2) &
!!$         *tau_dot_tau_tab(t2,t3,t5,t6)*delta(m3,m6)*delta(t1,t4) &
!!$    !Eq 2.3
!!$         + CT(0)*(4.d0*const(14))*mpi(impi)*sigma_dot_q_ope252 &
!!$         *(2.d0* tau_dot_tau_tab(t2,t1,t5,t4)*chp_sigma_dot_q_mtx(m1,m4,qtrans2)*delta(t3,t6)*delta(m3,m6) &
!!$         - tau1_dot_tauXtau_tab(t2,t3,t1,t5,t6,t4)*sigmaXsigma_dot_q(m3,m1,m6,m4,qtrans2) )
!!$
!!$!213
!!$   !Eq 2.1
!!$    cont_1PE = cont_1PE - 8.d0*const(14)*CT(0)*mpi(impi)*sigma_dot_q_ope252*chp_sigma_dot_q_mtx(m3,m6,qtrans2) &
!!$         *tau_dot_tau_tab(t2,t1,t5,t4)*delta(m1,m4)*delta(t3,t6) &
!!$    !Eq 2.3
!!$         + CT(0)*(4.d0*const(14))*mpi(impi)*sigma_dot_q_ope252 &
!!$         *(2.d0* tau_dot_tau_tab(t2,t3,t5,t6)*chp_sigma_dot_q_mtx(m3,m6,qtrans2)*delta(t1,t4)*delta(m1,m4) &
!!$         - tau1_dot_tauXtau_tab(t2,t1,t3,t5,t4,t6)*sigmaXsigma_dot_q(m1,m3,m4,m6,qtrans2) )
