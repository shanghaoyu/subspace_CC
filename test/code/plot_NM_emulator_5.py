...
from validation import prettyplease
...
#df_12k =  pd.read_pickle("/home/slime/subspace_CC/test/code/wave5_NM_fewbody/wave5_NM_fewbody_NI_IM1_cM3_batch_2.pickle")
#print(df_12k)
#print(df_12k.shape)
...
def txt_to_pickle():
    file_path = "./NM_ccd_800k_samples_batch_2.txt"
    raw_data  = np.loadtxt(file_path)

    for loop in np.where((raw_data[:,1]<=0.12) | (raw_data[:,1]>=0.20) |  (raw_data[:,4] < 0)  | (raw_data[:,5] < 0)|(raw_data[:,4]>80)|(raw_data[:,5]>800) ):
...
#        raw_data[loop,2:6] = np.nan
    ...
    df.to_pickle('NM_ccd_800k_samples_batch_2.pickle')

txt_to_pickle()

df_800k_NM     = pd.read_pickle("NM_ccd_800k_samples_batch_2.pickle")

##################
## read Li results
##################
Li6_E = np.load('./Li6_extrapolated_ground_state_energies_batch_2.npy')

print(Li6_E)
Li6_E = Li6_E[:,0]
print(Li6_E.shape)

df_800k_NM['E6Li'] = Li6_E
print(df_800k_NM)

#################
## drop NaN (saturation points outside 0.12-0.20)
#################
df_800k_NM_new = df_800k_NM.dropna(axis=0,how='any') 
print (df_800k_NM_new.shape)
...
# draw corner plot
#######################
io_1.plot_corner_plot_1(df_800k_NM_new.loc[:,['saturation_density', 'saturation_energy','symmetry_energy', 'L', 'K','E6Li']],6,weights_0)
...
### new corner plot
###################
#df_array = df_800k_NM_new.loc[:,['saturation_density', 'saturation_energy','symmetry_energy', 'L', 'K']].values
...
#sns.set_style("white")
#fig = prettyplease.corner(df_array, n_uncertainty_digits=2, labels=labels, plot_estimates=True, colors='cornflowerblue')
##axes = axes.reshape((5,5))
##axes[3,0].set_ylim(0,75)
#
#plt.savefig('custom_corner_plot.pdf', bbox_inches='tight')
