...
for loop1 in range(validation_count):
 ...
 #   print ("LEC="+str(LEC_random))
    for loop2 in range(dens_count):
        ...
        database_dir = "./emulator/DNNLO450/pnm_66_"+str(("%.2f" % dens))+"_DNNLOgo_20percent_64points/"
        emulator_cal, emulator_vec = emulator(database_dir,LEC_random,subtract_pnm)
        pnm_data[loop2] = emulator_cal.real/66
#    print("pnm: "+str(pnm_data*66))
    for loop2 in range(dens_count):
        dens = 0.12 + loop2 * 0.02
        dens = np.around(dens, 2 )
        database_dir = "./emulator/DNNLO450/snm_132_"+str(("%.2f" % dens))+"_DNNLOgo_20percent_64points/"
        emulator_cal, emulator_vec = emulator(database_dir,LEC_random,subtract_snm)
