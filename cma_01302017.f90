...
!!$    cnlo_pw(7) =-0.802300295338460D0
    ...
    CALL chp_setup_2PE_2loop_int_data(nof_2PE_2loop_int_x_points, nof_2PE_2loop_int_z_points, 2*mpi(0)/sfr)
...
!!$    c4_bar = c4_3nf - gA4*mpi(impi)/(16.d0*pi*fpi2)
    ...
    c1_bar = -0.13d-3 !gA2*mpi(impi)/(64.d0*pi*fpi2)
    c3_bar =  0.89d-3 !gA4*mpi(impi)/(16.d0*pi*fpi2)
    c4_bar =  0.d0
...
!See equation 2.17 from [ref1]
  ...
  function V3NF_F1(q2,loop_A,impi) 
...
!!$    INTEGER :: nx1, ny1, nz1, nx2, ny2, nz2, nx3, ny3, nz3, nx4, ny4, nz4, nx5, ny5, nz5 , nx6, ny6, nz6 
    ...
    REAL*8 :: delta, nucleon_mass, coeff
    real*8,parameter:: ct_bar = -0.06d-4
...
!! N3LO 3NF Relativistic Corrections 
  ...
  function rel1PE_3nf_corr(p,q,r,s,t,u) RESULT(res)
    ...
    real*8, parameter :: beta_8 = 0.25d0, beta_9 = 0d0, Ct_bar = -0.45*0.25d-4, Cs_bar = -4.19*0.25d-4
    REAL*8 :: delta, nucleon_mass, coeff
  ...
  function rel2PE_3nf_corr(p,q,r,s,t,u) RESULT(res)
    ...
    res = &
         ...
         *chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*( dot_product(qtrans3,kmean3)-dot_product(qtrans1,kmean1) ) &
         ...
         !
         -2.0*imag*(2.0*beta_9+1.0)*chp_sigma_dot_q_mtx(m3,m6,kmean3)*chp_sigma_dot_q_mtx(m2,m5,q1xq3)) &
         !
         ...
         !
         !
         +gA2/(32.0*mnuc(imnuc)*fpi4)*chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)/((q12+mpi2(impi))*(q12+mpi2(impi)))*&
  ...
  function ring_3NF(p,q,r,s,t,u) result(res)
    ...
    REAL*8 :: k1(3), k2(3), k3(3), k4(3), k5(3), k6(3), q1xq2(3), q2xq3(3), q1xq3(3)
    REAL*8 :: qtrans1(3), qtrans2(3), qtrans3(3), kmean1(3), kmean2(3),kmean3(3)
    REAL*8 ::  q12, q22, q32, Aq1, Aq2, Aq3, z, int_3pt
    REAL*8 :: delta, nucleon_mass, coeff
    ...
    if(q12 == 0.d0 .or. q32 == 0.d0) then
       ...
       z = dot_product(qtrans1,qtrans3)/(q12*q32)**(0.5)
    end if 
    
    int_3pt = ringIntegral(qtrans1,qtrans3,impi);
    !!    print*, int_3pt
    res = chp_sigma_dot_sigma_mtx(m1,m2,m4,m5)*tau_dot_tau_tab(t2,t3,t5,t6)&
         *R1_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m3,m6)*delta_tab(t1,t4)&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m2,m5,qtrans1)*tau_dot_tau_tab(t2,t3,t5,t6)&
         *R2_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m3,m6)*delta_tab(t1,t4)&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m2,m5,qtrans3)*tau_dot_tau_tab(t2,t3,t5,t6)&
         *R3_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m3,m6)*delta_tab(t1,t4)&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*chp_sigma_dot_q_mtx(m2,m5,qtrans1)*tau_dot_tau_tab(t2,t3,t5,t6)&
         *R4_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m3,m6)*delta_tab(t1,t4)&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*chp_sigma_dot_q_mtx(m2,m5,qtrans3)*tau_dot_tau_tab(t2,t3,t5,t6)&
         *R5_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m3,m6)*delta_tab(t1,t4)&
         !
         +tau_dot_tau_tab(t1,t3,t4,t6)*0.5*R6_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)&
         *delta_tab(m3,m6)*delta_tab(t2,t5)*delta_tab(m1,m4)*delta_tab(m2,m5)&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans1)&
         *R7_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t1,t4)*delta_tab(t2,t5)*delta_tab(t3,t6)&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)&
         *0.5*R8_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t1,t4)*delta_tab(t2,t5)*delta_tab(t3,t6)&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*chp_sigma_dot_q_mtx(m3,m6,qtrans1)&
         *0.5*R9_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t1,t4)*delta_tab(t2,t5)*delta_tab(t3,t6)&
         !
         +chp_sigma_dot_sigma_mtx(m1,m3,m4,m6)&
         *0.5*R10_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t1,t4)*delta_tab(t2,t5)*delta_tab(t3,t6)&
         !
         +chp_q_dot_qxsigma(qtrans1,qtrans3,m2,m5)*tau1_dot_tauXtau_tab(t1,t2,t3,t4,t5,t6)&
         *0.5*R11_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m1,m4)*delta_tab(m3,m6)
    !
    !
    !
!!$    res = res &
!!$         +tau_dot_tau_tab(t1,t2,t4,t5)*delta_tab(t3,t6)*delta_tab(m1,m4)*delta_tab(m2,m5)&
!!$         *S1_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m3,m6)&
!!$         !
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans1)*tau_dot_tau_tab(t1,t2,t4,t5)&
!!$         *S2_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t3,t6)&
!!$         !
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*chp_sigma_dot_q_mtx(m3,m6,qtrans1)*tau_dot_tau_tab(t1,t2,t4,t5)&
!!$         *S3_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t3,t6)&
!!$         !
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*tau_dot_tau_tab(t1,t2,t4,t5)&
!!$         *S4_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t3,t6)&
!!$         !
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*tau_dot_tau_tab(t1,t2,t4,t5)&
!!$         *S5_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t3,t6)&
!!$         !
!!$         +chp_sigma_dot_sigma_mtx(m1,m3,m4,m6)*tau_dot_tau_tab(t1,t2,t4,t5)&
!!$         *S6_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(t3,t6)&
!!$         !
!!$         +chp_q_dot_qxsigma(qtrans1,qtrans3,m1,m4)*tau1_dot_tauXtau_tab(t1,t2,t3,t4,t5,t6)&
!!$         *S7_3nf(q12,q22,q32,z,int_3pt,Aq1,Aq2,Aq3,impi)*delta_tab(m2,m5)*delta_tab(m3,m6)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
    if(res .ne. res) then
       print*, res
       print*,'Integral = ', int_3pt
       print*, 'qtrans1 ', qtrans1
       print*,'qtrans3 ',qtrans3
    end if
 ...
 !       print*,"ERRRRRRRRRRRRRRROOOOOOOORRRRRRRRRRRRR"
...
!!$            /( 16.d0*pi*(betaF**2.d0 + 4.d0*alphaF*gammaF) )
   ...
   function R1_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
     ...
     eps = 1.0d-3     
     if(q12 < eps) q12 = q12 + eps
     if(q22 < eps) q22 = q22 + eps
     if(q32 < eps) q32 = q32 + eps
     
     if(abs(1.0-z2) < eps) z2 = z2 + eps
     ...
     r1_3nf = 0d0
!!$     eps = 1.d-3
!!$     denom = (4.0*(z2-1.0)*mpion2-q22)*(4.0*mpion2*q3+q33)
!!$     if(denom .eq. 0d0) denom = eps
!!$     r1_3nf = r1_3nf + coeff*(z2-1.0)*mpion*( 2.0*mpion2 + q32 )*( q22*q3 + 4.0*mpion2*(z*q1+q3) )/denom 
!!$     !
!!$     denom = (z2-1.d0)*q1*q32
!!$     if(denom .eq. 0d0) denom = eps
!!$     r1_3nf = r1_3nf - Aq2*coeff*q22*(2.d0*mpion2*(q1+z*q3)+z*q3*(q32-q12))/denom
!!$     !
!!$     denom = (z2-1.d0)*q1*q3
!!$     if(denom .eq. 0d0) denom = eps
!!$     r1_3nf = r1_3nf -coeff*Aq3*(z*q22*(z*q1-q3)*q3+2.d0*mpion2*(z*(z2-2.d0)*q12-(1.d0+z2)*q1*q3-z*q32))&
!!$          /denom 
!!$     !
!!$     denom = ((z2-1.d0)*q32)
!!$     if(denom .eq. 0d0) denom = eps
!!$     r1_3nf = r1_3nf + coeff*Aq1*(2.d0*mpion2*q22+q3*(-z*q13+(2.0-3.0*z2)*q12*q3-z*(z2-2.0)*q1*q32+q33))/denom 
!!$     !
!!$     denom = (z2-1.d0)*(4.d0*(z2-1.d0)*mpion2-q22)*q3
!!$     if(denom .eq. 0d0) denom = eps
!!$     r1_3nf = r1_3nf - Int_3pt*q22*4.d0*coeff/denom &
!!$          *(8.0*(z2-1.0)*mpion4*(2.0*z*q1+(1.0+z2)*q3)+q22*q3*(z2*q12+z*(z2-1.0)*q1*q3-q32) &
!!$          +2.0*mpion2*(z*(z2-2.0)*q13-(1.0+2.0*z2)*q12*q3+3.0*z*(z2-2.0)*q1*q32+(2.0*z2*z2-3.0)*q33))

   r1_3nf = (gA**6*mpion*(2*mpion2+q32)*(-1 +z2)*(4*mpion2*(q3 + q1*z) + q3*(q12 + q32 + 2*q1*q3*z)))/(128.*fpi**6*pi*(4*mpion2*q3 + q33)*(-q12 - q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2)))&
        !
        - (gA**6*Int_3pt*(q12 + q32 + 2*q1*q3*z)*(q3*(q12 + q32 + 2*q1*q3*z)*(-q32 + q12*z2 + q1*q3*z*(-1 +z2)) + 8*mpion4*(-1 +z2)*(2*q1*z + q3*(1 + z2)) + 2*mpion2*(q13*z*(-2 + z2) + 3*q1*q32*z*(-2 + z2) - q12*q3*(1 + 2*z2) + q33*(-3 + 2*z2**2))))/(32.*fpi**6*q3*(-1 +z2)*(-q12 - q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2))) + (gA**6*(2*mpion2*(q12 + q32 + 2*q1*q3*z) + q3*(q33 -q13*z + q12*q3*(2 - 3*z2) - q1*q32*z*(-2 + z2)))*atan(q1/(2.*mpion)))/(256.*fpi**6*pi*q1*q32*(-1 + z2))&
 !
 - (gA**6*(q3*z*(-q3 + q1*z)*(q12 + q32 + 2*q1*q3*z) + 2*mpion2*(-(q32*z) + q12*z*(-2 + z2) - q1*q3*(1 +z)))*atan(q3/(2.*mpion)))/(256.*fpi**6*pi*q1*q32*(-1 +z2))&
 !
 - (gA**6*sqrt(q12 + q32 + 2*q1*q3*z)*(q3*(-q12 + q32)*z + 2*mpion2*(q1 + q3*z))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))/(256.*fpi**6*pi*q1*q32*(-1 + z2))
     
     
     r1_3nf = R1_3nf/(hc)**5.0
     
   end function R1_3nf
  
!!$  function R2_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
!!$    
!!$    use constants, only : hbarc
!!$    
!!$    implicit none 
...
!!$    r2_3nf = 0.d0 
!!$    eps = 1.d-4
!!$    denom = (z2-1.0)**2.0*q13*q32
!!$!    if(denom .eq. 0d0) denom = eps
!!$    r2_3nf = r2_3nf +coeff*Aq2*q22*(-2.0*mpion2*((1.d0+z2)*q1 + 2.d0*z*q3)&
!!$         +z*q3*( (1.0 + z2)*q12 - 2.d0*q32))/denom
!!$    !
!!$    
!!$    denom = (z2-1.d0)**2.d0*q13*q3
!!$ !   if(denom .eq. 0d0) denom = eps
!!$    r2_3nf = r2_3nf+(coeff*2.d0)*Aq3*(mpion2*(2.0*z*q12+(1.0+3.0*z2)*q1*q3+2.0*z*q32) &
!!$         +z*q3*(-z*q13-z2*q12*q3+z*q1*q32+q33))/denom
!!$    
!!$    !
!!$    denom = (z2-1.0)**2.0*q12*q32
!!$  !  if(denom .eq. 0d0) denom = eps
!!$    r2_3nf = r2_3nf+Aq1*coeff*(2.0*mpion2*( (1.0+z2)*q12 + z*(3.0+z2)*q1*q3 + (1.0+z2)*q32)&
!!$         +q3*(-(z+z2*z)*q13 + (2.0-5.0*z2+z2*z2)*q12*q3 + z*(1.0+z2)*q1*q32 + (1.0+z2)*q33))/denom
!!$    
!!$    !
!!$    denom = (z2-1.d0)**2.0*q12*(-4.0*(-1.0+z2)*mpion2+q22)*q3
!!$   ! if(denom .eq. 0d0) denom = eps
!!$    r2_3nf = r2_3nf - Int_3pt*4.0*coeff/denom*(q23*q2*q3*( -2.0*z2*q12+(1.0+z2)*q32 )-&
!!$         8.d0*(z-1.0)*(1.0+z)*mpion4*(z*(2.0+z2)*q13 + (1.0+2.0*z2)**2.0*q12*q3 + z*(2.0 + 7.0*z2)*q1*q32 + (1.0+2.0*z2)*q33)+&
!!$         2.0*mpion2*q22*(2.0*z*q13 + (1.0-z2 + 6.0*z2*z2)*q12*q3 - 2.0*z*(-1.0-3.0*z2+z2*z2)*q1*q32 + (3.0+3.0*z2-4.0*z2*z2)*q33))
!!$    
!!$    !
!!$    denom = q12*(4.d0*(z2-1.d0)*mpion2-q22)*(4.0*mpion2*q3+q33)
!!$!    if(denom .eq. 0d0) denom = eps
!!$    r2_3nf = r2_3nf + mpion*coeff*(2.d0*mpion2+q32)*(q22*q3+4.d0*mpion2*(z*q1+q3))/denom
!!$    
!!$    r2_3nf = (gA**6*mpion*(2*mpion2 +q32)*(4*mpion2*(q3 + q1*z) + q3*(q12 + q32 + 2*q1*q3*z)))/(128.*fpi**6*pi*q12*(4*mpion2*q3 +q33)*(-q12 -q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2)))&
!!$        !
!!$        - (gA**6*Int_3pt*(q12 + q32 + 2*q1*q3*z)*(q3*(q12 + q32 + 2*q1*q3*z)*(-q32 + q12*z2 + q1*q3*z*(-1 +z2)) + 8*mpion4*(-1 +z2)*(2*q1*z + q3*(1 + z2)) + 2*mpion2*(q13*z*(-2 + z2) + 3*q1*q32*z*(-2 + z2) - q12*q3*(1 + 2*z2) + q33*(-3 + 2*z2**2))))/(32.*fpi**6*q3*(-1 +z2)*(-q12 - q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2))) + (gA**6*(2*mpion2*(q12 + q32 + 2*q1*q3*z) + q3*(q33 -q13*z + q12*q3*(2 - 3*z2) - q1*q32*z*(-2 + z2)))*atan(q1/(2.*mpion)))/(256.*fpi**6*pi*q1*q32*(-1 + z2))&
!!$ !
!!$ - (gA**6*(q3*z*(-q3 + q1*z)*(q12 + q32 + 2*q1*q3*z) + 2*mpion2*(-(q32*z) + q12*z*(-2 + z2) - q1*q3*(1 +z)))*atan(q3/(2.*mpion)))/(256.*fpi**6*pi*q1*q32*(-1 +z2))&
...
!!$    r2_3nf = (gA**6*mpion*(2*mpion2 +q32)*(4*mpion2*(q3 + q1*z) + q3*(q12 + q32 + 2*q1*q3*z)))/(128.*fpi**6*pi*q12*(4*mpion2*q3 +q33)*(-q12 -q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2)))&
!!$!         
!!$- (gA**6*Int_3pt*(q3*(q12 + q32 + 2*q1*q3*z)**2*(-2*q12*z2 + q32*(1 +z2) ) - 8*mpion4*(-1 + z)*(1 + z)*(q13*z*(2 + z2) +q33*(1 + 2*z2) + q12*q3*(1 + 2*z2)**2 + q1*q32*z*(2 + 7*z2)) + 2*mpion2*(q12 + q32 + 2*q1*q3*z)*(2*q13*z + q33*(3 + 3*z2 - 4*z2**2) - 2*q1*q32*z*(-1 - 3*z2 + z2**2) + q12*q3*(1 -z2 + 6*z2**2))))/(32.*fpi**6*q12*q3*(-1 + z2)**2*(q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2)))&
!!$!
!!$ + (gA**6*(2*mpion2*(q12*(1 + z2) + q32*(1 + z2) + q1*q3*z*(3 + z2)) + q3*(q33*(1 + z2) + q1*q32*z*(1 + z2) + q13*(-z - z**3) + q12*q3*(2 - 5*z2 + z2**2)))*atan(q1/(2.*mpion)))/(256.*fpi**6*pi*q13*q32*(-1 + z2)**2)&
!!$!
!!$ + (gA**6*(q3*z*(q33 - q13*z + q1*q32*z - q12*q3*z2) + mpion2*(2*q12*z + 2*q32*z + q1*q3*(1 + 3*z2)))*atan(q3/(2.*mpi)))/(128.*fpi**6*pi*q13*q32)*(-1 + z2)**2 &
!!$! 
!!$+ (gA**6*sqrt(q12 +q32+ 2*q1*q3*z)*(-2*mpion2*(2*q3*z + q1*(1 + z2)) + q3*z*(-2*q32 +q12*(1 + z2)))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))/(256.*fpi**6*pi*q13*q32*(-1 + z2)**2);

    
    r2_3nf = r2_3nf/(hc)**7.0
    
  end function R2_3nf
  
  function R3_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
...
!!$    denom = (z2-1.d0)**2.d0*q12*q33
!!$!    if(denom .eq. 0d0) denom = eps    
!!$    R3_3nf = r3_3nf-z*Aq2*coeff*q22*(-4.0*mpion2*(q1+z*q3)+q3*(2.0*z*q12 + (z2-1.d0)*q1*q3 - 2.0*z*q32))/denom
!!$    
!!$    !
!!$    denom = q12*q32*(z2-1.0)**2.0
!!$ !   if(denom .eq. 0d0) denom = eps        
!!$    r3_3nf = r3_3nf - z*Aq3*coeff*(mpion2*(-2.0*z*(-3.0 + z2)*q12 + 4.0*(1.0+z2)*q1*q3 + 4.0*z*q32) + q3*(-(z2+1.0)*q13- &
!!$         2.0*z2*z*q12*q3 + (1.0+z2)*q1*q32 + 2.0*z*q33))/denom
!!$    
!!$    !
!!$    denom = q1*q33*(z2-1.0)**2.0
!!$!    if(denom .eq. 0d0) denom = eps        
!!$    r3_3nf = r3_3nf-z*Aq1*coeff*(2.0*mpion2*(2.0*q12 + 4.0*z*q1*q3 + (1.0+z2)*q32) + q3*(-2.0*z*q13 + &
!!$         (1.0-3.0*z2)*q12*q3 + 2.0*z*q1*q32 + (1.0 + z2)*q33))/denom
!!$    
!!$    !
!!$    denom = q1*(z2-1.0)**2.0*(-4.0*(z2-1.0)*mpion2+q22)*q32
!!$ !   if(denom .eq. 0d0) denom = eps    
!!$    r3_3nf = r3_3nf-Int_3pt*4.0*coeff*z/denom * ( q23*q2*q3*( (1.0+z2)*q12 + z*(z2-1.0)*q1*q3 - (1.0+z2)*q32)+&
!!$         8.0*(z - 1.0) * (1.0+z)*mpion4*(3.0*z*q13 + (10.0*z2-1.0)*q12*q3 + 3.0*z*(1.0 + 2.0*z2)*q1*q32 + (1.0+2.0*z2)*q33)+&
!!$         2.0*mpion2*q22*(z*(-3.0 + z2)*q13 + (3.0-9.0*z2)*q12*q3 - z*(5.0 + z2)*q1*q32 + (-3.0 - 3.0*z2 + 4.0*z2*z2)*q33))
!!$    
!!$    !
!!$    denom = q1*(-4.0*(z2-1.0)*mpion2+q22)*q32*(4.0*mpion2+q32)
!!$  !  if(denom .eq. 0d0) denom = eps    
!!$    r3_3nf=r3_3nf+ z*coeff*mpion*(2.0*mpion2+q32)*(q22*q3+4.0*mpion2*(z*q1+q3))/denom
!!$    
!!$    R3_3nf =  (gA**6*mpion*(2*mpion2 + q32)*z*(4*mpion2*(q3 + q2*z) + q3*(q22 + q32 + 2*q2*q3*z)))/(128.*fpi**6*pi*q2*q32*(4*mpion2 + q32)*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2))) &
!!$!
!!$- (gA**6*Int_3pt*z*(q3*(q22 +q32 + 2*q2*q3*z)**2*(q2*q3*z*(-1 + z2) + q22*(1 + z2) - q32*(1 + z2)) + 8*mpion4*(-1 + z)*(1 + z)*(3*q23*z +q3*(1 + 2*z2) + 3*q2*q32*z*(1 + 2*z2) + q22)*q3*(-1 + 10*z2)) + 2*mpion2*(q22 +q32 + 2*q2*q3*z)*(q22*q3*(3 - 9*z2) + q23*z*(-3 + z2) - q2*q32*z*(5 + z2) + q33*(-3 - 3*z2 + 4*z2**2))))/(32.*fpi**6*q2*q32*(-1 +z2)**2*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2))) &
!!$!
!!$- (gA**6*z*(2*mpion2*(2*q22 + 4*q2*q3*z +q32*(1 + z2)) + q3*(-2*q23*z + 2*q2*q32*z +q22*q3*(1 - 3*z2) + q33*(1 +z2)))*atan(q2/(2.*mpion)))/(256.*fpi**6*pi*q22*q33*(-1 + z2)**2) &
!!$!
!!$- (gA**6*z*(mpion2*(4*q32*z - 2*q22*z*(-3 + z2) + 4*q2*q3*(1 + z2)) + q3*(2*q33*z - 2*q22*q3*z**3 + q23*(-1 -z2) + q2*q32*(1 + z2)))*atan(q3/(2.*mpion)))/(256.*fpi**6*pi*q22*q33*(-1 + z2)**2) &
!!$!
!!$- (gA**6*z*sqrt(q22 + q32 + 2*q2*q3*z)*(-4*mpion2*(q2 + q3*z) + q3*(2*q22*z - 2*q32*z + q2*q3*(-1 + z2)))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(256.*fpi**6*pi*q22*q33*(-1 + z2)**2)

    
    R3_3nf = R3_3nf/(hc**7.0)
    
  end function R3_3nf
  
  function R4_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
    implicit none 
    ...
    eps = 1.d-3
    R4_3nf=0d0
!!$    denom = q12*q33*(z2-1.d0)**2.d0
!!$   ! if(denom .eq. 0d0) denom = eps    
!!$    R4_3nf = R4_3nf + Aq2*coeff*q22*(-2.0*z2*q12*q3 + (1.0+z2)*q33 + 2.0*mpion2*(2.0*z*q1 + (1.0+z2)*q3))/denom
!!$    !
!!$    
!!$    denom = q1*q33*(z2-1.d0)**2.d0
!!$    !if(denom .eq. 0d0) denom = eps        
!!$    R4_3nf=R4_3nf + Aq1*coeff*(-2.0*mpion2*(2.0*z*q12+(1.0+3.0*z2)*q1*q3+2.0*z*q32)+q3*(2.*z2*q13 + 2.0*z2*z*q12*q3 &
!!$         +(1.0-4.0*z2+z2*z2)*q1*q32-2.0*z*q33))/denom
!!$    
!!$    !
!!$    denom = q12*q32*(z2-1.0)**2.0
!!$   ! if(denom .eq. 0d0) denom = eps    
!!$    R4_3nf=R4_3nf-Aq3*coeff*(2.0*mpion2*(-z2*(-3.0+z2)*q12 + z*(3.0+z2)*q1*q3 + (1.0+z2)*q32)+&
!!$         q3*(-z*(1.0+z2)*q13 - (1.0-z2 + 2.0*z2*z2)*q12*q3 + z*(1.0+z2)*q1*q32 + (1.0+z2)*q33))/denom
!!$    
!!$    !
!!$    denom = q1*q32*(-4.0*(z2-1.0)*mpion2+q22)*(z2-1.0)**2.0
!!$    !if(denom .eq. 0d0) denom = eps    
!!$    R4_3nf=R4_3nf-Int_3pt*4.0*coeff/denom*( q22*q22*q3*( (1.0+z2)*q12*z +(z2-1.0)**2.0*q1*q3 - 2.0*z*q32)+&
!!$         8.0*(z-1.d0)*(1.0 + z)*mpion4*( 3.0*z2*q13 + 9.0*z2*z*q12*q3 + (-2.0 + 9.0*z2 + 2.0*z2*z2)*q1*q32 + z*(2.0+z2)*q33)+&
!!$         2.0*mpion2*q22*(z2*(z2-3.0)*q13 + (2.0*z-8.0*z2*z)*q12*q3 + (4.0+5.0*z2*(z2-3.0))*q1*q32 + 2.0*z*(-3. + z2 + z2*z2)*q33))
!!$    
!!$    !
!!$    denom = q1*q32*(-4.0*(z2-1.d0)*mpion2+q22)*(4.0*mpion2+q32)
!!$    if(denom .eq. 0d0) denom = eps    
!!$    R4_3nf=R4_3nf+z*coeff*mpion*(2.0*mpion2+q32)*(q22*q3 + 4.0*mpion2*(z*q1+q3))/denom
   
!!$    R4_3nf = (gA**6*mpion*(2*mpion2 + q32)*z*(4*mpion2*(q3 + q2*z) + q3*(q22 + q32 + 2*q2*q3*z)))/(128.*fpi**6*pi*q2*q32*(4*mpion2 +q32)*(q22 + q3 + 2*q2*q3*z - 4*mpion2*(-1 +z2))) &
!!$!
!!$- (gA**6*Int_3pt*(q3*(q22 + q32 + 2*q2*q3*z)**2*(-2*q32*z + q2*q3*(-1 + z2)**2 + q22*(z + z**3)) + 8*mpion4*(-1 + z)*(1 + z)*(3*q23*z2 + 9*q22*q3*z**3 +q33*z*(2 + z2) + q2*q32*(-2 + 9*z2+ 2*z2**2)) + 2*mpion2*(q22 + q32 + 2*q2*q3*z)*(q23*z2*(-3 + z2) + q22*q3*(2*z - 8*z**3) + 2*q33*z*(-3 +z2 + z2**2) + q2*q32*(4 + 5*z2*(-3 + z2)))))/(32.*fpi**6*q2*q32*(-1 + z2)**2)*(q22 +q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2))) &
!!$!
!!$+ (gA**6*(-2*mpion2*(2*q22*z + 2*q32*z + q2*q3*(1 + 3*z2)) + q3*(-2*q33*z + 2*q23*z2 + 2*q22*q3*z**3 + q2*q32*(1 - 4*z2 + z2**2)))*atan(q2/(2.*mpion)))/(256.*fpi**6*pi*q22*q33*(-1 + z2)**2)&
!!$!
!!$ - (gA**6*(2*mpion2*(-(q22*z2*(-3 + z2)) + q32*(1 + z2) + q2*q3*z*(3 +z2)) + q3*(q33*(1 +z2) + q2*q32*z*(1 +z2) + q23*(-z - z**3) - q22*q3*(1 -z2 + 2*z2**2)))*atan(q3/(2.*mpion)))/(256.*fpi**6*pi*q22*q33*(-1 + z2)**2)&
!!$!
!!$ + (gA**6*sqrt(q22 +q32 + 2*q2*q3*z)*(-2*q22*q3*z2 + q33*(1 + z2) + 2*mpion2*(2*q2*z + q3*(1 +z2)))*atan(sqrt(q22 +q32 + 2*q2*q3*z)/(2.*mpion)))/(256.*fpi**6*pi*q22*q33*(-1 + z2)**2);
  ...
  function R5_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    ...
    R5_3nf = 0d0
!!$    denom = q1*q32*q32*(-1.0+z2)**2.0
!!$    if(denom .eq. 0d0) denom = eps    
!!$ !   if(abs(denom) < eps) denom = eps
!!$    R5_3nf  = R5_3nf + Aq2*coeff*q22*(-4.0*mpion2*(q1 + z*q3) + q3*(2.0*z*q12 + (-1.0+z2)*q1*q3 - 2.0*z*q32))/denom
!!$    if(R5_3nf .ne. R5_3nf)then
!!$       print*,"R5 = ", R5_3nf
!!$       print*,"denom : ", denom
!!$       print*,"q12, q22, q32, z : ",q12,q22,q32,z
!!$       STOP
!!$    end if 
!!$    !
!!$    denom = q1*q33*(-1.0 + z2)**2.0
!!$    if(denom .eq. 0d0) denom = eps        
!!$    R5_3nf = R5_3nf - Aq3*coeff/denom*( 2.0*mpion2*( z*(z2 - 3.)*q12 - 2.0*(1.+z2)*q1*q3 - 2.0*z*q32) + q3*( (1.+z2)*q13+&
!!$         2.0*z**3.0 *q12*q3 - (1. + z2)*q1*q32 - 2.0*z*q33) )
!!$    
!!$    !
!!$    denom = q32*q32*(z2 - 1.0)**2.0
!!$  !  if(denom .eq. 0d0) denom = eps        
!!$    R5_3nf = R5_3nf + Aq1*coeff*(2.*mpion2*(2.*q12 + 4.0*z*q1*q3 + (1.+z2)*q32) + q3*(-2.*z*q13 +(1.-3.*z2)*q12*q3 + 2.*z*q1*q32 + (1.+z2)*q33) )/denom
!!$    
!!$    !
!!$    denom = (z2-1.0)**2.0*(-4.0*(z2-1.0)*mpion2+q22)*q33
!!$    if(denom .eq. 0d0) denom = eps        
!!$    R5_3nf = R5_3nf + Int_3pt*4.0*coeff/denom*(q22**2.*q3*( (1.0+z2)*q12 + z*(z2-1.0)*q1*q3 - (1.+z2)*q32)+&
!!$         8.0*(z-1.0)*(1.0+z)*mpion4*(3.*z*q13 + (10.*z2-1.)*q12*q3 + 3.*z*(1.+2.*z2)*q1*q32 + (1. + 2.*z2)*q33)+ &
!!$         2.0*mpion2*q22*(z*(z2-3.0)*q13 + (3.- 9.*z2)*q12*q3 - z*(5.0+z2)*q1*q32 + (4.0*z2*z2 - 3.0*z2 - 3.0)*q33)) 
!!$    
!!$    
!!$    !
!!$    denom = (-4.0*(z2 - 1.0)*mpion2 + q22)*q33*(4.0*mpion2 + q32)
!!$    if(denom .eq. 0d0) denom = eps    
!!$    R5_3nf = R5_3nf - coeff*mpion*(2.0*mpion2 + q32)*(q22*q3 + 4.*mpion2*(z*q1 + q3))/denom

!!$    
!!$    R5_3nf = -(gA**6*mpion*(2*mpion2 + q32)*(4*mpion2*(q3 + q2*z) + q3*(q22 + q32 + 2*q2*q3*z)))/(128.*fpi**6*pi*q33*(4*mpion2 + q32)*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 +z2)))&
!!$!
!!$ + (gA**6*Int_3pt*(q3*(q22 + q32 + 2*q2*q3*z)**2*(q2*q3*z*(-1 +z2) + q22*(1 +z2) - q32*(1 + z2)) + 8*mpion4*(-1 + z)*(1 + z)*(3*q23*z + q33*(1 + 2*z2) + 3*q2*q32*z*(1 + 2*z2) + q22*q3*(-1 + 10*z2)) + 2*mpion2*(q22 +q32 + 2*q2*q3*z)*(q22*q3*(3 - 9*z2) + q23*z*(-3 + z2) - q2*q32*z*(5 + z2) + q33*(-3 - 3*z2 + 4*z2**2))))/(32.*fpi**6*q33*(-1 +z2)**2*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2)))&
!!$!
!!$ + (gA**6*(2*mpion2*(2*q22 + 4*q2*q3*z + q32*(1 + z2)) + q3*(-2*q23*z + 2*q2*q32*z +q22*q3*(1 - 3*z2) +q33*(1 +z2)))*atan(q2/(2.*mpion)))/(256.*fpi**6*pi*q2*q32**2*(-1 +z2)**2)&
!!$!
!!$ - (gA**6*(2*mpion2*(-2*q32*z + q22*z*(-3 +z2) - 2*q2*q3*(1 + z2)) + q3*(-2*q33*z + 2*q22*q3*z**3 +q23*(1 +z2) - q2*q32*(1 +z2)))*atan(q3/(2.*mpion)))/(256.*fpi**6*pi*q2*q3**4*Power(-1 + z2)**2)&
!!$!
!!$ + (gA**6*sqrt(q22 +q32 + 2*q2*q3*z)*(-4*mpion2*(q2 + q3*z) + q3*(2*q22*z - 2*q32*z + q2*q3*(-1 + z2)))*atan(sqrt(q22 +q32 + 2*q2*q3*z)/(2.*mpion)))/(256.*fpi**6*pi*q2*q3**4*Power(-1 +z2)**2);

    
    
    R5_3nf = R5_3nf/(hc**7.0)
    
  end Function R5_3nf
  
  function R6_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    ...
    q32 = qtrans32/(hc)**2.0
!!$    eps = 1.0d-6     
!!$    if(q12 .eq. 0d0) q12 = q12 + eps
!!$    if(q22 .eq. 0d0) q22 = q22 + eps
!!$    if(q32 .eq. 0d0) q32 = q32 + eps
    
!!$    if(abs(1.0-z2) .eq. 0d0) z2 = z2 + eps
...
!    eps = 1.d-3
    R6_3nf = 0.d0
    !
!!$    R6_3nf = R6_3nf + Aq2*coeff*(2.0*mpion2+q22) &
!!$         +Aq1*coeff*(2.*z*q1*q3+8.*mpion2+3.0*q12+q32)&
!!$         +Aq3*coeff*(2.*z*q1*q3+8.*mpion2+q12+3.*q32)&
!!$         -8.*coeff*Int_3pt*mpion2*(2.0*mpion2+q22)&
!!$         +coeff/(4.*mpion*(4.*mpion2+q12)*(4.*mpion2+q32))*(288.*mpion2**3.-q12*q22*q32+8.*mpion4*(9.*(q12+q32)-4.*q1*q3*z)&
!!$         -2.*mpion2*(q12**2-7.*q12*q32+q32**2+4.*q1*q3*(q12+q32)*z))
    !
!!$    R6_3nf = R6_3nf + Aq1*coeff*(2.0*z*(mpion2 + q12)*q3 + q1*(8.0*mpion2 + 3.0*q12 + q32) )/q1
!!$    if(q1 .ne. 0d0) R6_3nf = R6_3nf + Aq1*coeff/q1*2.*mpion2*q3 - 2.*coeff*mpion3*q3*z/(q1*(4.*mpion2+q12))
!!$    if(q3 .ne. 0d0) R6_3nf = R6_3nf + Aq3*coeff/q3*2.*z*q1*mpion2 - 2.*coeff*mpion3*q1*z/(q3*(4.*mpion2+q32))
    !
!!$    R6_3nf = R6_3nf+Aq3*coeff/q3*(2.0*z*q1*(mpion2 + q32)+q3*(8.*mpion2 + q12 + 3.0*q32))
    !
!!$    denom = q1*(-4.0*(z2-1.0)*mpion2+q22)*q3
!!$   ! if(denom .eq. 0d0) denom = eps        
!!$    R6_3nf = R6_3nf - Int_3pt*coeff*4.0*(2.0*mpion2 + q22)/denom*&
!!$         (q1*q22*q3*(q12 + z*q1*q3 + q32) + 4.0*mpion4*(z*q12 - 2.0*(z2-2.0)*q1*q3 + z*q32)&
!!$         +2.0*mpion2*(4.0*q1*q3*(q12+q32)+z*(q12**2 + 6.0*q12*q32 + q32**2) ) )

!!$    denom = 4.*mpion2*(-1.0+z2)-q22
!!$    if(abs(denom) > 1d-15) then
!!$       print*,denom
!!$       R6_3nf = R6_3nf + 4.0*coeff*Int_3pt*(2.*mpion2+q22)/denom*(8.*mpion4+q22*(q12+q32+q1*q3*z)&
!!$            +2.*mpion2*(4.*q12-q22+4.*q32+6.*q1*q3*z))
!!$    end if
!!$    denom = q3*(4.*mpion2*(-1.0+z2)-q22)
!!$    if(abs(denom) > 1d-15) then
!!$       print*,denom
!!$       R6_3nf = R6_3nf + 8.*coeff*Int_3pt*mpion2*q1*(2.0*mpion2+q12)*(2.*mpion2+q22)*z/denom
!!$    end if
!!$    denom = q1*(4.*mpion2*(-1.0+z2)-q22)
!!$    if(abs(denom) > 1d-15) then
!!$       print*,denom
!!$       R6_3nf = R6_3nf + 8.*coeff*Int_3pt*mpion2*q3*(2.0*mpion2+q32)*(2.*mpion2+q22)*z/denom
!!$    end if 
    !
!!$    denom = q1*(4.0*mpion2 + q12)*(4.0*(z2-1.0)*mpion2 - q22)*q3*(4.0*mpion2 + q32)
!!$    !if(denom .eq. 0d0) denom = eps            
!!$    R6_3nf = R6_3nf - coeff*mpion/denom*&
!!$         ( (5.0 + z2)*q13*q22*q33 + 8.0*mpion4*mpion2*(z*(4.0*z2 - 3.0)*q12 + 2.0*(19.0 - 18.0*z2)*q1*q3 + z*(4.0*z2 - 3.0)*q32)+&
!!$         2.0*mpion4*(4.0*z*(z2-1.)*q12**2. + (77. - 36.*z2)*q13*q3 + 2.*z*(33. + 8.*z2)*q12*q32+&
!!$         (77. - 36.*z2)*q1*q33 + 4.*z*(z2-1.)*q32*q32) + 2.*mpion2*q1*q3*( (10.+z2)*q12**2. + 2.*z*(9.+2.*z2)*q13*q3 + (29.-7.*z2)*q12*q32 &
!!$         +2.*z*(9.+2.*z2)*q1*q33 + (10. + z2)*q32**2.) )

!!$    denom = 4.*mpion*(4.*mpion2+q12)*(4.*mpion2+q32)*(4.*mpion2*(-1.+z2)-q22)
!!$    if(abs(denom) > 1d-15) then
!!$       print*,denom
!!$       R6_3nf = R6_3nf + coeff/denom*(-64.*mpion4**2-q12*q22**2*q32-2.*mpion2*q22*(q12**2+5.*q12*q32+q32**2.+4.*q1*q3*(q12+q32)*z)&
!!$            +8.*mpion4*(-(q12+q32)*(11.*(q12+q32)-9.*q22)-2.*q1*q3*(11.*(q12+q32)+2.*q22)*z)&
!!$            -8.*mpion3**2*(-36.*q22+41.*q32+41*q1*(q1+2.*q3*z)))
!!$    end if
!!$       
!!$    denom = (4.*mpion2+q12)*q3*(4.0*mpion2+q32)*(4.0*mpion2*(-1.+z2)-q22)
!!$    if(abs(denom) > 1d-15) then
!!$       print*,denom
!!$       R6_3nf = R6_3nf -2.0*coeff*mpion3*q1/denom*(4.*mpion4+(4.*mpion2+q12)*q22)*z
!!$    end if 
!!$    
!!$    denom = (4.*mpion2+q12)*q1*(4.0*mpion2+q32)*(4.0*mpion2*(-1.+z2)-q22)
!!$    if(abs(denom) > 1d-15) then
!!$       print*,denom
!!$       R6_3nf = R6_3nf -2.0*coeff*mpion3*q3/denom*(4.*mpion4+(4.*mpion2+q32)*q22)*z
!!$    end if 

!!$    R6_3nf =  -(gA**6*Int_3pt*(2*mpion2 +q22 +q32 + 2*q2*q3*z)*(q2*q3*(q22 +q32 + q2*q3*z)*(q22 +q32 + 2*q2*q3*z) + 2*mpion2*(4*q2*q3*(q22 +q32) + (q2**4 + 6*q22*q3**2 +q3**4)*z) + 4*mpion4*(q22*z + q32*z - 2*q2*q3*(-2 +z2))))/(32.*fpi**6*q2*q3*(q22 +q32 + 2*q2*q3*z - 4*mpion2*(-1 +z2))) &
!!$!
!!$- (gA**6*mpion*(q23*q33*(q22 + q32 + 2*q2*q3*z)*(5 +z2) + 2*mpion2*q2*q3*(q22*q32*(29 - 7*z2) + q2**4*(10 +z2) +q3**4*(10 +z2) + 2*q23*q3*z*(9 + 2*z2) + 2*q2*q33*z*(9 + 2*z2)) + 8*mpion**6*(2*q2*q3*(19 - 18*z2) +q22*z*(-3 + 4*z2) + q32*z*(-3 + 4*z2)) + 2*mpion4*(q23*q3*(77 - 36*z2) + q2*q33*(77 - 36*z2) + 4*q2**4*z*(-1 +z2) + 4*q3**4*z*(-1 +z2) + 2*q22*q32*z*(33 + 8*z2))))/(128.*fpi**6*pi*q2*(4*mpion2 +q22)*q3*(4*mpion2 + q32)*(-q22 -q32 - 2*q2*q3*z + 4*mpion2*(-1 +z2)))&
!!$!
!!$ + (gA**6*(q2*(8*mpion2 + 3*q22 +q32) + 2*(mpion2 +q22)*q3*z)*atan(q2/(2.*mpion)))/(256.*fpi**6*pi*q22)&
!!$!
!!$ + (gA**6*(q3*(8*mpion2 + q22 + 3*q32) + 2*q2*(mpion2 + q32)*z)*atan(q3/(2.*mpion)))/(256.*fpi**6*pi*q32) &
!!$!
!!$+ (gA**6*(2*mpion2 +q22 +q32 + 2*q2*q3*z)*atan(sqrt(q22 +q32 + 2*q2*q3*z)/(2.*mpion)))/(256.*fpi**6*pi*sqrt(q22 +q32 + 2*q2*q3*z)))/2.);

    
    R6_3nf = R6_3nf/(hc**5.0)
    
  end function R6_3nf
    
  function R7_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
    implicit none 
...
!!$    denom = q12*(-4.0*(z2-1.0)*mpion2 + q22)
!!$    !if(denom .eq. 0d0) denom = eps        
!!$    R7_3nf =R7_3nf + coeff*mpion*(2.0*mpion2 + q22)/denom
!!$    
!!$    !
!!$    denom = q13*(z2-1.0)**2.0
!!$    !if(denom .eq. 0d0) denom = eps    
!!$    R7_3nf = R7_3nf - coeff*Aq3*(2.0*mpion2 + q22)*( (1.0+z2)*q1 + 2.*z*q3)/denom
!!$    
!!$    !
!!$    denom = q12*q3*(-1.0+z2)**2.0
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R7_3nf = R7_3nf -coeff*Aq1*(2.0*mpion2 + q22)*(2.0*z*q1 + (1.+z2)*q3)/denom
!!$    
!!$    !
!!$    denom = q13*q3*(z2-1.0)**2.0
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R7_3nf = R7_3nf +coeff*Aq2*(2.0*mpion2 + q22)*(2.0*z*q12 + (1.0 + 3.0*z2)*q1*q3 + 2.0*z*q32)/denom
!!$    
!!$    !
!!$    denom = q12*(z2-1.0)**2.0*(4.0*(z2-1.0)*mpion2 - q22)
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R7_3nf = R7_3nf + 4.0*coeff*Int_3pt*(2.0*mpion2 + q22)/denom*&
!!$         (-q22*( (1.0+z2)*q12 + z*(3.0+z2)*q1*q3 + (1.+z2)*q32 )+&
!!$         4.0*(z2-1.0)*mpion2* ( (1.+2.*z2)*q12 + 2.*z*(2. + z2)*q1*q3 + (1. + 2.*z2)*q32))
!!$    
!!$    R7_3nf=   (3*gA**6*mpion*(2*mpion2 +q22 +q32 + 2*q2*q3*z))/(256.*fpi**6*pi*q22*(q22 +q32 + 2*q2*q3*z - 4*mpion2*(-1 +z2)))&
!!$!
!!$ + (3*gA**6*Int_3pt*(2*mpion2 +q22 +q32 + 2*q2*q3*z)*((-q22 -q32 - 2*q2*q3*z)*(q22*(1 +z2) +q32*(1 +z2) + q2*q3*z*(3 +z2)) + 4*mpion2*(-1 +z2)*(2*q2*q3*z*(2 +z2) +q22*(1 + 2*z2) +q32*(1 + 2*z2))))/(64.*fpi**6*q22*(-1 + z2)**2*(-q22 -q32 - 2*q2*q3*z + 4*mpion2*(-1 +z2)))&
!!$!
!!$ - (3*gA**6*(2*mpion2 +q22 +q32 + 2*q2*q3*z)*(2*q2*z + q3*(1 +z2))*atan(q2/(2.*mpion)))/(512.*fpi**6*pi*q23*q3*(-1 +z2)**2) &
!!$!
!!$- (3*gA**6*(2*mpion2 +q22 +q32 + 2*q2*q3*z)*(2*q3*z + q2*(1 +z2))*atan(q3/(2.*mpion)))/(512.*fpi**6*pi*q23*q3*(-1 +z2)**2) &
!!$!
!!$+ (3*gA**6*(2*mpion2 +q22 +q32 + 2*q2*q3*z)*(2*q22*z + 2*q32*z + q2*q3*(1 + 3*z2))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(512.*fpi**6*pi*q23*q3*sqrt(q22 +q32 + 2*q2*q3*z)*(-1 + z2)**2);

    
    R7_3nf = R7_3nf/(hc**7.0)
    
  end function R7_3nf
  
  function R8_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
    implicit none 
    real*8 :: q1, q2,q3,Aq1,Aq2,Aq3
    ...
    eps = 1.d-3
    R8_3nf = 0d0
!!$    denom = q1*(-4.*(z2-1.)*mpion2 + q22)*q3
!!$    !if(denom .eq. 0d0) denom = eps    
!!$    R8_3nf =R8_3nf - coeff*z*mpion*(2.0*mpion2 + q22)/denom
!!$    
!!$    !
!!$    denom = q12*q3*(z2-1.0)**2
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R8_3nf=R8_3nf +coeff*z*Aq3*(2.0*mpion2 + q22)*( (1.0+z2)*q1 + 2.0*z*q3)/denom
!!$    
!!$    !
!!$    denom = q1*q32*(z2-1.0)**2
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R8_3nf = R8_3nf + coeff*z*Aq1*(2.*mpion2 + q22)*(2.*z*q1 + (1. + z2)*q3)/denom
!!$
!!$    !
!!$    denom = q12*q32*(z2-1.0)**2
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R8_3nf = R8_3nf - coeff*z*Aq2*(2.*mpion2 + q22)*(2.0*z*q12 + (1.+3.*z2)*q1*q3 + 2.0*z*q32)/denom
!!$    
!!$    !
!!$    denom = q1*(z2-1.0)**2.0*(4.0*(z2-1.0)*mpion2-q22)*q3
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R8_3nf = R8_3nf - 4.0*coeff*Int_3pt*z*(2.0*mpion2 + q22)/denom*&
!!$         (-q22*( (1.+z2)*q12 + z*(3.+z2)*q1*q3 + (1.0+z2)*q32)+&
!!$         4.0*(z2-1.0)*mpion2*( (1.+2.*z2)*q12 + 2.0*z*(2.0+z2)*q1*q3 + (1.+2.*z2)*q32))

!!$    R8_3nf = ((-3*gA**6*mpion*z*(2*mpion2 +q22 +q32 + 2*q2*q3*z))/(256.*fpi**6*pi*q2*q3*(q22 +q32 + 2*q2*q3*z - 4*mpion2*(-1 +z2))) &
!!$!         
!!$         - (3*gA**6*Int_3pt*z*(2*mpion2 +q22 + q32 + 2*q2*q3*z)*((-q22 -q32 - 2*q2*q3*z)*(q22*(1 +z2) +q32*(1 + z2) + q2*q3*z*(3 + z2))&
!!$         + 4*mpion2*(-1 +z2)*(2*q2*q3*z*(2 +z2) +q22*(1 + 2*z2) + q32*(1 + 2*z2))))/(64.*fpi**6*q2*q3*(-1 + z2)**2*(-q22 -q32 - 2*q2*q3*z + 4*mpion2*(-1 + z2)))&
!!$         !
!!$         + (3*gA**6*z*(2*mpion2 +q2**2 +q32 + 2*q2*q3*z)*(2*q2*z + q3*(1 + z2))*atan(q2/(2.*mpion)))/(512.*fpi**6*pi*q22*q32*(-1 + z2)**2)&
!!$!
!!$         + (3*gA**6*z*(2*mpion2 +q22 +q32 + 2*q2*q3*z)*(2*q3*z + q2*(1 +z2))*atan(q3/(2.*mpion)))/(512.*fpi**6*pi*q22*q32*(-1 +z2)**2)&
!!$         !
!!$         - (3*gA**6*z*(2*mpion2 + q22 +q32 + 2*q2*q3*z)*(2*q22*z + 2*q32*z + q2*q3*(1 + 3*z2))*atan(sqrt(q22 +q32 + 2*q2*q3*z)/(2.*mpion)))&
!!$         /(512.*fpi**6*pi*q22*q32*sqrt(q22 +q32 + 2*q2*q3*z)*Power(-1 +z2)**2))/2.;

    
    R8_3nf = R8_3nf/(hc**7.0)
    
  end function R8_3nf
  
  function R9_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
...
!!$    denom = q12*q32*(z2-1.0)**2.0
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R9_3nf = R9_3nf - coeff/denom*Aq2*(2.0*mpion2 + q22)*( (1.0+z2)*q12 + z*(3.0+z2)*q1*q3 + (1.0+z2)*q32)
!!$    
!!$    !
!!$    denom = q1*q32*(z2-1.0)**2.0
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R9_3nf=R9_3nf +coeff/denom*Aq1*( (1.0+z2)*q13 + 2.0*z*(2.0+z2)*q12*q3 - z2*(z2-7.0)*q1*q32 + 2.0*z*q33 + 2.0*mpion2*( (1.0+z2)*q1 + 2.0*z*q3 ) )
!!$    
!!$    !
!!$    denom = q12*q3*(z2-1.0)**2.
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R9_3nf=R9_3nf+coeff*Aq3/denom*(2.*z*q13 - z2*(-7.+z2)*q12*q3+ 2.*z*(2.+z2)*q1*q32 + (1.0+z2)*q33 +2.0*mpion2*( 2.0*z*q1 + (1.+z2)*q3 ) )
!!$
!!$    !
!!$    denom = q1*(-1+z2)**2.0*(-4.0*(z2-1.0)*mpion2 + q22)*q3
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R9_3nf = R9_3nf + 4.0*coeff*Int_3pt*z*(2.0*mpion2 + q22)/denom * (q22*(-2.0*q12 + z*(z2-5.0)*q1*q3 - 2.0*q32)+&
!!$         4.0*(z2-1.0)*mpion2*( (2.0+z2)*q12 + 6.0*z*q1*q3 + (2.0+z2)*q32) )
!!$    
!!$    !
!!$    denom = q1*(-4.0*(z2-1.0)*mpion2 + q22)*q3 
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R9_3nf = R9_3nf - coeff*mpion*z*(2.0*mpion2 + q22)/denom
!!$    R9_3nf=  ((-3*gA**6*mpion*z*(2*mpion2 + q22 + q32 + 2*q2*q3*z))/(256.*fpi**6*pi*q2*q3*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2))) &
!!$!
!!$         + (3*gA**6*Int_3pt*z*(2*mpion2 + q22 + q32 + 2*q2*q3*z)*((q22 + q32 + 2*q2*q3*z)*(-2*q22 - 2*q32 + q2*q3*z*(-5 + z2)) &
!!$         + 4*mpion2*(-1 + z2)*(6*q2*q3*z + q22*(2 + z2) + q32*(2 + z2))))/(64.*fpi**6*q2*q3*(-1 + z2)**2*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2))) &
!!$!
!!$         + (3*gA**6*(2*q33*z - q2*q32*z2*(-7 + z2) + q23*(1 + z2) + 2*q22*q3*z*(2 + z2) + 2*mpion2*(2*q3*z + q2*(1 + z2)))*atan(q2/(2.*mpion)))&
!!$         /(512.*fpi**6*pi*q22*q32*(-1 + z2)**2) &
!!$         !
!!$         + (3*gA**6*(2*q23*z - q22*q3*z2*(-7 + z2) + q33*(1 + z2) + 2*q2*q32*z*(2 + z2) + 2*mpion2*(2*q2*z + q3*(1 + z2)))*atan(q3/(2.*mpion)))/(512.*fpi**6*pi*q22*q32*(-1 + z2)**2) &
!!$!
!!$         - (3*gA**6*(2*mpion2 + q22 + q32 + 2*q2*q3*z)*(q22*(1 + z2) + q32*(1 + z2) + q2*q3*z*(3 + z2))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))&
!!$         /(512.*fpi**6*pi*q22*q32*sqrt(q22 + q32 + 2*q2*q3*z)*(-1 + z2)**2))/2.;

    !
    R9_3nf = R9_3nf/(hc**7.0)
    
  end function R9_3nf
  
  function R10_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
...
!!$    denom = -4.0*(z2-1.0)*mpion2 + q22
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R10_3nf =R10_3nf+ coeff*(z2 - 1.0)*mpion*(2.0*mpion2 + q22)/denom
!!$    
!!$    !
!!$    denom = q1*q3*(z2-1.0)
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R10_3nf = R10_3nf + coeff*Aq2*(2.0*mpion2 + q22)*(z*q1 + q3)*(q1 + z*q3)/denom 
!!$    
!!$    !
!!$    denom = q3*(z2-1.0)
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R10_3nf = R10_3nf - coeff*Aq1*(z*q13 + (1.0 + 2.0*z2)*q12*q3 - z*(z2-4.)*q1*q32 + q33 + 2.*mpion2*(z*q1 + q3) )/denom
!!$    
!!$    !
!!$    denom = (z2-1.0)*q1
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R10_3nf=R10_3nf -coeff*Aq3*(q13-z*(z2-4.0)*q12*q3 + (1.0+2.0*z2)*q1*q32 + z*q33 + 2.0*mpion2*(q1+z*q3))/denom
!!$    
!!$    !
!!$    denom = (z2-1.0)*(4.0*(z2-1.0)*mpion2 - q22)
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R10_3nf = R10_3nf + 4.0*coeff*Int_3pt*(2.0*mpion2 + q22)/denom*( -q22*( q12 - z*(z2-3.0)*q1*q3 + q32)+ &
!!$         4.0*(z2-1.0)*mpion2*( (1.0+z2)*q12 + 4.0*z*q1*q3 + (1.0+z2)*q32 ) )

!!$    R10_3nf =   ((3*gA**6*mpion*(2*mpion2 + q22 + q32 + 2*q2*q3*z)*(-1 + z2))/(256.*fpi**6*pi*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2))) &
!!$!
!!$         + (3*gA**6*Int_3pt*(2*mpion2 + q22 + q32 + 2*q2*q3*z)*((-q22 - q32 - 2*q2*q3*z)*(q22 + q32 - q2*q3*z*(-3 + z2)) + 4*mpion2*(-1 + z2)&
!!$         *(4*q2*q3*z + q22*(1 + z2) + q32*(1 + z2))))/(64.*fpi**6*(-1 + z2)*(-q22 - q32 - 2*q2*q3*z + 4*mpion2*(-1 + z2)))&
!!$!
!!$         - (3*gA**6*(q33 + q23*z + 2*mpion2*(q3 + q2*z) - q2*q32*z*(-4 + z2) + q22*q3*(1 + 2*z2))*atan(q2/(2.*mpion)))/(512.*fpi**6*pi*q2*q3*(-1 + z2)) &
!!$!
!!$         - (3*gA**6*(q23 + q33*z + 2*mpion2*(q2 + q3*z) - q22*q3*z*(-4 + z2) + q2*q32*(1 + 2*z2))*atan(q3/(2.*mpion)))/(512.*fpi**6*pi*q2*q3*(-1 + z2)) &
!!$!
!!$         + (3*gA**6*(q3+q2*z)*(q2+q3*z)*(2*mpion2+q22 +q32 +2*q2*q3*z)*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(512.*fpi**6*pi*q2*q3*sqrt(q22 + q32 + 2*q2*q3*z)*(-1 + z2)))/2.;

    
    
    R10_3nf = R10_3nf/(hc**5.0)
    
  end function R10_3nf
  
  function R11_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
    ...
    R11_3nf = 0d0
!!$    denom = (z2-1.0)*q12*q32
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R11_3nf = R11_3nf - coeff*Aq2*q22*(4.0*mpion2 + q12 + q32)/denom
!!$    
!!$    !
!!$    denom = q12*q32*(z2-1.0)
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R11_3nf = R11_3nf + coeff*Aq3*(2.0*mpion2*( (z2-1.0)*q12 + 2.0*z*q1*q3 + 2.0*q32) + q3*(z*q13 + (2.0*z2-1.0)*q12*q3 + z*q1*q32 + q33) )/denom
!!$    
!!$    !
!!$    denom = q12*q32*(z2-1.0)
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R11_3nf = R11_3nf + coeff*Aq1*(2.0*mpion2*(2.0*q12 + 2.0*z*q1*q3 + (z2-1.0)*q32) + q1*(q13 + z*q12*q3 + (2.0*z2-1.0)*q1*q32 + z*q33) )/denom
!!$    
!!$    !
!!$    denom = (z2-1.0)*q12*(-4.0*(z2-1.0)*mpion2 + q22)*q32
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R11_3nf = R11_3nf - 4.0*coeff*Int_3pt*q22/denom*(-(2.0*mpion2 + q12)*(2.0*mpion2 + q32)*(4.0*mpion2 + q12 + q32)+&
!!$         2.0*z2*z*q1*q3*(-4.0*mpion4 + q12*q32) + z2*(4.0*mpion2 + q12 + q32)*(4.0*mpion4 + 3.0*q12*q32 + 2.0*mpion2*(q12+q32) ) + &
!!$         z*q1*q3*(8.0*mpion4 + q12*q12 + q32*q32 + 4.0*mpion2*(q12 + q32) ) )
!!$    
!!$    !
!!$    denom = q12*(4.0*mpion2 + q12)*(-4.0*(z2-1.0)*mpion2 + q22)*q32*(4.0*mpion2 + q32)
!!$    !if(denom .eq. 0d0) denom = eps
!!$    R11_3nf = R11_3nf - coeff*mpion/denom*(-2.0*z2*( 4.0*mpion4*q12*( 4.0*mpion2 + q12 )+&
!!$         4.0*mpion2*(2.0*mpion2 + q12)**2.0 *q32 + (2.0*mpion2 + q12)**2.0 *q32*q32 ) + 2.0*mpion2*(4.0*mpion2 + q12 + q32)*( q12*q32 + 2.0*mpion2*(q12+q32))-&
!!$         z*q1*q3*(32.0*mpion4*mpion2 + 12.0*mpion4*(q12 + q32) + q12*q32*(q12 + q32) + 2.0*mpion2*(q12*q12 + 4.0*q12*q32 + q32*q32 ) ) )

!!$    R11_3nf =  (-(gA**6*mpion*(2*mpion2*(4*mpion2 + q22 + q32)*(q22*q32 + 2*mpion2*(q22 + q32)) - q2*q3*(32*mpion**6 + 12*mpion**4*(q22 + q32)&
!!$         + q22*q32*(q22 + q32) + 2*mpion2*(q2**4 + 4*q22*q32 +q3**4))*z - 2*(4*mpion**4*q22*(4*mpion2 + q22)&
!!$         + 4*mpion2*(2*mpion2 + q22)**2*q32 +(2*mpion2 + q22)**2*q3**4)*z2))/(256.*fpi**6*pi*q22*(4*mpion2 + q22)*q32*(4*mpion2 + q32)*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2)))&
!!$!
!!$         - (gA**6*Int_3pt*(q22 + q32 + 2*q2*q3*z)*((-2*mpion2 - q22)*(2*mpion2 + q32)*(4*mpion2 + q22 + q32) + q2*q3*(8*mpion4 +q2**4 +q3**4 &
!!$         + 4*mpion2*(q22 + q32))*z + (4*mpion2 + q22 + q32)*(4*mpion4 + 3*q22*q32 + 2*mpion2*(q22 + q32))*z2 &
!!$         + 2*q2*q3*(-4*mpion4 + q22*q32)*z**3))/(64.*fpi**6*q22*q32*(-1 + z2)*(q22 + q32 + 2*q2*q3*z - 4*mpion2*(-1 + z2))) &
!!$!
!!$         + (gA**6*(2*mpion2*(2*q22 + 2*q2*q3*z + q32*(-1 + z2)) + q2*(q23 + q22*q3*z + q33*z + q2*q32*(-1 + 2*z2)))*atan(q2/(2.*mpion)))/(512.*fpi**6*pi*q23*q32*(-1 + z2)) &
!!$         !
!!$         + (gA**6*(2*mpion2*(2*q32 + 2*q2*q3*z + q22*(-1 + z2)) + q3*(q33 + q23*z + q2*q32*z + q22*q3*(-1 + 2*z2)))*atan(q3/(2.*mpion)))/(512.*fpi**6*pi*q22*q33*(-1 + z2)) &
!!$!
!!$         - (gA**6*(4*mpion2 + q22 + q32)*sqrt(q22 + q32 + 2*q2*q3*z)*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(512.*fpi**6*pi*q22*q32*(-1 + z2)))/2.;
    
    !
    R11_3nf = R11_3nf/(hc**7.0)
  end function R11_3nf

  function S1_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
    
    implicit none 
    real*8 :: q1, q2,q3,z,Aq1,Aq2,Aq3
    real*8 :: S1_3nf, int_3pt,qtrans12,qtrans22,qtrans32,ringInt,loopAq1,loopAq2,loopAq3
    real*8 :: z2,q12,q22,q32, coeff,q13,q23,q33,mpion2,mpion4,mpion,mpion3,eps,denom,hc
    integer :: impi
    z2 = z*z
...
!!$    S1_3nf = &
!!$         -Aq1*coeff*(2.0*mpion2 + q12) - Aq2*coeff*(4.0*mpion2 + q12 + z*q1*q3 + q32)-coeff*Aq3*(2.0*mpion2 + q32) &
!!$         !
!!$         +4.0*coeff*Int_3pt*(2.0*mpion2 + q12)*(2.0*mpion2 + q32) - 2.0*coeff*mpion
    S1_3nf = (-(gA**4*mpion)/(32.*fpi**6*pi) + (gA**4*Int_3pt*(2*mpion2 + q22)*(2*mpion2 + q32))/(16.*fpi**6) - (gA**4*(2*mpion2 + q22)*atan(q2/(2.*mpion)))/(128.*fpi**6*pi*q2) - (gA**4*(2*mpion2 + q32)*atan(q3/(2.*mpion)))/(128.*fpi**6*pi*q3) - (gA**4*(4*mpion2 + q22 + q32 + q2*q3*z)*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(128.*fpi**6*pi*sqrt(q22 + q32 + 2*q2*q3*z)))/2.;

    S1_3nf = S1_3nf/(hc**5.0)
  end function S1_3nf

  function S2_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
...
!!$    denom = (z2-1.0)**2*q1
!!$    
!!$    S2_3nf = S2_3nf -Aq1*coeff*( (1.0+z2)*q1 + 2.0*z*q3)/denom
!!$    
!!$    !
!!$    denom = (z2-1.0)**2*q12
!!$    S2_3nf = S2_3nf - Aq3*coeff*q3*(2.0*z*q1 + (1.0+z2)*q3)/denom
!!$    
!!$    !
!!$    denom = q1*(z2-1.0)**2
!!$    S2_3nf = S2_3nf + Int_3pt*4.0*coeff*q3*(-4.0*z*(z2-1.0)*mpion2 + 2.0*z*q12 + (1.0+3.0*z2)*q1*q3 + 2.0*z*q32)/denom
!!$    
!!$    !
!!$    denom = q12*(z2-1.0)**2.0
!!$    S2_3nf = S2_3nf + Aq2*coeff*( (1.0 + z2)*q12 + z*(3.0 + z2)*q1*q3 + (1.0 + z2)*q32)/denom
    
    S2_3nf = ((gA**4*Int_3pt*q3*(2*q2**2*z + 2*q32*z - 4*mpion2*z*(-1 + z2) + q2*q3*(1 + 3*z2)))/(16.*fpi**6*q2*(-1 + z2)**2) &
         - (gA**4*(2*q3*z + q2*(1 + z2))*atan(q2/(2.*mpion)))/(128.*fpi**6*pi*q2**2*(-1 + z2)**2) &
         - (gA**4*(2*q2*z + q3*(1 + z2))*atan(q3/(2.*mpion)))/(128.*fpi**6*pi*q2**2*(-1 + z2)**2) &
         + (gA**4*(q2**2*(1 + z2) + q32*(1 + z2) + q2*q3*z*(3 + z2))*atan(sqrt(q2**2 + q32 + 2*q2*q3*z)/(2.*mpion)))/(128.*fpi**6*pi*q2**2*sqrt(q2**2 + q32 + 2*q2*q3*z)*(-1 + z2)**2))/2.;


    !
    S2_3nf = S2_3nf/(hc**7.0)
    
  end function S2_3nf

function S3_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
    use constants, only : hbarc
...
!!$  denom = q1*(z2-1.0)**2.0
!!$  
!!$  S3_3nf = S3_3nf + Aq3*coeff*( (1.0+z2)*q1 + 2.0*z*q3)/denom
!!$
!!$  !
!!$  denom = q3*(z2-1.0)**2.0
!!$  
!!$  S3_3nf=S3_3nf + Aq1*coeff*(2.0*z*q1 + (1.0+z2)*q3)/denom
!!$  
!!$  !
!!$  denom = q1*q3*(z2-1.0)**2
!!$  
!!$  S3_3nf = S3_3nf + z*Aq2*coeff*(-2.0*q12 + z*(z2-5.0)*q1*q3 - 2.0*q32)/denom
!!$  
!!$  !
!!$  denom = (z2-1.0)**2.0
!!$  
!!$  S3_3nf = S3_3nf - Int_3pt*4.0*coeff*(-4.0*(z2-1.0)*mpion2 + (1.0+z2)*q12 + z*(3.0+z2)*q1*q3 + (1.+z2)*q32)/denom
  S3_3nf= (-(gA**4*Int_3pt*(-4*mpion2*(-1 + z2) + q22*(1 + z2) + q32*(1 + z2) + q2*q3*z*(3 + z2)))/(16.*fpi**6*(-1+z2)**2) &
       + (gA**4*(2*q2*z + q3*(1 + z2))*atan(q2/(2.*mpion)))/(128.*fpi**6*pi*q2*q3*(-1+z2)**2) &
       + (gA**4*(2*q3*z + q2*(1 + z2))*atan(q3/(2.*mpion)))/(128.*fpi**6*pi*q2*q3*(-1+z2)**2) &
       + (gA**4*z*(-2*q22 - 2*q32 + q2*q3*z*(-5 + z2))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(128.*fpi**6*pi*q2*q3*sqrt(q22 + q32 + 2*q2*q3*z)*(-1+z2)**2))/2.;

  
  S3_3nf = S3_3nf/(hc**7.0)
  
end function S3_3nf

function S4_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
  use constants, only : hbarc
  ...
  S4_3nf = 0d0
!!$  denom = q3*(z2-1.0)**2.0
!!$  S4_3nf =S4_3nf+ z*Aq1*coeff*( (1.0+z2)*q1 + 2.0*z*q3)/denom
!!$  
!!$  !
!!$  denom = q1*(z2-1.0)**2.0
!!$  S4_3nf = S4_3nf+ z*Aq3*coeff*(2.0*z*q1 + (1.0+z2)*q3)/denom
!!$  
!!$  !
!!$  denom = (z2-1.0)**2
!!$  S4_3nf = S4_3nf + Int_3pt*z*4.0*coeff*(4.0*z*(z2-1.0)*mpion2 - 2.0*z*q12 &
!!$       -(1.0+3.0*z2)*q1*q3 - 2.0*z*q32)/denom
!!$  
!!$  !
!!$  denom = q1*q3*(z2-1.0)**2.0
!!$  S4_3nf=S4_3nf-z*Aq2*coeff*( (1.0 + z2)*q12 + z*(3.0 + z2)*q1*q3 & 
!!$       + (1.0 + z2)*q32)/denom
S4_3nf=((gA**4*Int_3pt*z*(-2*q22*z - 2*q32*z + 4*mpion2*z*(-1 + z2) - q2*q3*(1 + 3*z2)))/(16.*fpi**6*(-1+z2)**2) &
     + (gA**4*z*(2*q3*z + q2*(1 + z2))*atan(q2/(2.*mpion)))/(128.*fpi**6*pi*q2*q3*(-1+z2)**2) &
     + (gA**4*z*(2*q2*z + q3*(1 + z2))*atan(q3/(2.*mpion)))/(128.*fpi**6*pi*q2*q3*(-1+z2)**2)&
     - (gA**4*z*(q22*(1 + z2) + q32*(1 + z2) + q2*q3*z*(3 + z2))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(128.*fpi**6*pi*q2*q3*sqrt(q22 + q32 + 2*q2*q3*z)*(-1+z2)**2))/2.;
  
  S4_3nf = S4_3nf/(hc**7.0)
  
end function S4_3nf

function S5_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
  use constants, only : hbarc
...
!!$  denom = q32*(z2-1.0)**2.0
!!$  
!!$  S5_3nf = S5_3nf -Aq1*coeff*q1*( (1.0+z2)*q1 + 2.0*z*q3)/denom
!!$  
!!$  !
!!$  denom = q3*(z2-1.0)**2.0
!!$  
!!$  S5_3nf = S5_3nf - Aq3*coeff*(2.0*z*q1 + (1.0+z2)*q3)/denom
!!$  
!!$  !
!!$  denom = q3*(z2-1.0)**2
!!$  
!!$  S5_3nf = S5_3nf + Int_3pt*4.0*coeff*q1*(-4.*z*(z2-1.0)*mpion2 + 2.*z*q12 &
!!$       +(1.+3.*z2)*q1*q3 + 2.0*z*q32)/denom
!!$
!!$  !
!!$  denom = q32*(z2-1.0)**2.0
!!$  
!!$  S5_3nf=S5_3nf + Aq2*coeff*( (1.0+z2)*q12 + z*(3.0+z2)*q1*q3 + (1.0+z2)*q32)/denom

  S5_3nf= ((gA**4*Int_3pt*q2*(2*q22*z + 2*q32*z - 4*mpion2*z*(-1 + z2) + q2*q3*(1 + 3*z2)))/(16.*fpi**6*q3*(-1+z2)**2) &
       - (gA**4*(2*q3*z + q2*(1 + z2))*atan(q2/(2.*mpion)))/(128.*fpi**6*pi*q32*(-1+z2)**2) &
       - (gA**4*(2*q2*z + q3*(1 + z2))*atan(q3/(2.*mpion)))/(128.*fpi**6*pi*q32*(-1+z2)**2) &
       + (gA**4*(q22*(1 + z2) + q32*(1 + z2) + q2*q3*z*(3 + z2))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(128.*fpi**6*pi*q32*sqrt(q22 + q32 + 2*q2*q3*z)*(-1+z2)**2))/2.;

  
  
  S5_3nf=S5_3nf/(hc**7.0)
  
  end function S5_3nf

function S6_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
  use constants, only : hbarc
  implicit none 
  real*8 :: q1, q2,q3,Aq1,Aq2,Aq3
...
!!$  S6_3nf = &
!!$       -Aq3*coeff*q3*(z*q1 + q3)/(z2-1.0) &
!!$       !
!!$       + Aq2*coeff*(q12 - z*(z2-3.0)*q1*q3 + q32)/(z2-1.0) &
!!$       !
!!$       - Aq1*coeff*q1*(q1 + z*q3)/(z2-1.0) &
!!$       !
!!$       +Int_3pt*4.0*coeff*q1*q3*(z*q1 + q3)*(q1 + z*q3)/(z2-1.0)

S6_3nf = ((gA**4*Int_3pt*q2*q3*(q3 + q2*z)*(q2 + q3*z))/(16.*fpi**6*(-1 + z2)) &
     - (gA**4*(q2 + q3*z)*atan(q2/(2.*mpion)))/(128.*fpi**6*pi*(-1 + z2)) &
     - (gA**4*(q3 + q2*z)*atan(q3/(2.*mpion)))/(128.*fpi**6*pi*(-1 + z2)) &
     + (gA**4*(q22 + q32 - q2*q3*z*(-3 + z2))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(128.*fpi**6*pi*sqrt(q22 + q32 + 2*q2*q3*z)*(-1 + z2)))/2.;

  
  
  S6_3nf = S6_3nf/(hc**5.0)
  
end function S6_3nf

function S7_3nf(qtrans12,qtrans22,qtrans32,z,ringInt,loopAq1,loopAq2,loopAq3,impi)
  use constants, only : hbarc
...
!!$  S7_3nf = S7_3nf + &
!!$       Aq1*coeff*(2.0*mpion2 + q32)/((z2-1.0)*q32) &
!!$       !
!!$       -Int_3pt*coeff*4.0*(z*q1+q3)*(2.0*mpion2 + q32)/(q3*(z2-1.0))
!!$  
!!$  S7_3nf = S7_3nf&
!!$       !
!!$       - Aq2*coeff*(z*q32*(z*q1+q3)+2.0*mpion2*(q1+z*q3))/(q1*q32*(z2-1.0)) + &
!!$       !
!!$       z*Aq3*coeff*(2.0*mpion2+q32)/(q1*q3*(z2-1.0))
  
  S7_3nf= (-(gA**4*Int_3pt*(2*mpion2 + q32)*(q3 + q2*z))/(32.*fpi**6*q3*(-1 + z2)) &
       + (gA**4*(2*mpion2 + q32)*atan(q2/(2.*mpion)))/(256.*fpi**6*pi*q2*q32*(-1 + z2))&
       + (gA**4*(2*mpion2 + q32)*z*atan(q3/(2.*mpion)))/(256.*fpi**6*pi*q2*q32*(-1 + z2))&
       - (gA**4*(q32*z*(q3 + q2*z) + 2*mpion2*(q2 + q3*z))*atan(sqrt(q22 + q32 + 2*q2*q3*z)/(2.*mpion)))/(256.*fpi**6*pi*q2*q32*sqrt(q22 + q32 + 2*q2*q3*z)*(-1 + z2)))/2.;

  
  
  S7_3nf = S7_3nf/(hc**7.0)
end function S7_3nf
...
!!$       !! XXXXXXX  New Stuff XXXXXXX  !!!!
         ...
         !!chp_N3LO_2PE_Wc_1loop_ci1 (q2, loop_w, loop_L, 2, imnuc_2PE) + &
         !chp_N3LO_2PE_Wc_1loop_ci0 (kmean2, q2, loop_w, loop_L,2, imnuc_2PE) + &
         ! 
         
         VLS = 0.d0
         !VLS = chp_spin_dot_qxk_mtx(m1,m2,m3,m4,qxk)*delta(t1,t3)*delta(t2,t4)*&
          !    chp_N3LO_2PE_VLS_1loop_ci1(loop_w, loop_L,2, imnuc_2PE) + &
              !chp_N3LO_2PE_VLS_1loop_ci0(q2, loop_w, loop_L,2,imnuc_2PE)
         WLS = 0.d0
        ! WLS = chp_spin_dot_qxk_mtx(m1,m2,m3,m4,qxk)*chp_tau_dot_tau_mtx(t1,t2,t3,t4)*&
         !     (chp_N3LO_2PE_WLS_1loop_ci1(q2, loop_w, loop_L,2, imnuc_2PE) + &
         ...
         !chp_N3LO_2PE_VT_1loop_ci0 (kmean2, q2, loop_w, loop_L,2, imnuc_2PE) + &
         ! 
         ...
         !
         !chp_N3LO_2PE_Vs_1loop_ci0 (kmean2, q2, loop_w, loop_L, 2, imnuc_2PE) +&
        ...
        ! VsigL = chp_N3LO_2PE_VsigL_1loop_ci0(loop_L,2, imnuc_2PE)*&
         !     chp_sigma1_dot_q_sigma2_dot_q_mtx(m1,m2,m3,m4,qxk)*delta(t1,t3)*delta(t2,t4)
...
!!$    if ( nz1 + nz2 /= nz3 + nz4 ) return 
  ...
  complex*16 function chiral_3nf(p,q,r,s,t,u)
    ...
    REAL*8 :: q22, q32, q12, freg12, freg22, freg32, Aq1, Aq2, Aq3
    ...
    impi = 2
!!$!123    
!!$    vn3lo_2PE = vn3lo_2PE + vn3lo2PE(p,q,r,s,t,u)
!!$!132
...
!!$!123
!!$    vn3lo_2P1PE = sigma_dot_q_ope363*&
...
!!$
!!$!132
!!$    vn3lo_2P1PE = vn3lo_2P1PE + sigma_dot_q_ope252*&
!!$         (tau_dot_tau_tab(t1,t2,t4,t5)*(chp_sigma_dot_q_mtx(m3,m6,qtrans1)*sum(qtrans1*qtrans2)*V3NF_F1(Q12,Aq1,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3NF_F2(Q12,Aq1,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3nf_F3(Q12,Aq1,impi))&
!!$         *delta_tab(m1,m4)*delta_tab(t3,t6) &
!!$         +tau_dot_tau_tab(t3,t2,t6,t5)*( (chp_sigma_dot_q_mtx(m1,m4,qtrans1)*sum(qtrans1*qtrans2)*V3NF_F4(Aq1) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans2)*V3NF_F5(Q12,Aq1) )*delta(m3,m6)*delta(t1,t4) &
!!$         +(chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3NF_F6(Q12,Aq1,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3NF_F7(Q12,Aq1,impi) )*delta(m1,m4)*delta(t1,t4)) &
!!$         +chp_tau1Xtau2_dot_tau3(t1,t3,t2,t4,t6,t5)*sigmaXsigma_dot_q(m1,m3,m4,m6,qtrans1)*V3NF_F8(Q12,Aq1,impi) )
!!$
!!$!312
!!$    vn3lo_2P1PE = vn3lo_2P1PE + sigma_dot_q_ope252*&
!!$         (tau_dot_tau_tab(t3,t2,t6,t5)*(chp_sigma_dot_q_mtx(m1,m4,qtrans3)*sum(qtrans3*qtrans2)*V3NF_F1(Q32,Aq3,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*V3NF_F2(Q32,Aq3,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans2)*V3nf_F3(Q32,Aq3,impi))&
!!$         *delta_tab(m3,m6)*delta_tab(t1,t4) &
!!$         +tau_dot_tau_tab(t1,t2,t4,t5)*( (chp_sigma_dot_q_mtx(m3,m6,qtrans3)*sum(qtrans3*qtrans2)*V3NF_F4(Aq3) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3NF_F5(Q32,Aq3) )*delta(m1,m4)*delta(t3,t6) &
!!$         +(chp_sigma_dot_q_mtx(m1,m4,qtrans3)*V3NF_F6(Q32,Aq3,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans2)*V3NF_F7(Q32,Aq3,impi) )*delta(m3,m6)*delta(t3,t6)) &
!!$         +chp_tau1Xtau2_dot_tau3(t3,t1,t2,t6,t4,t5)*sigmaXsigma_dot_q(m3,m1,m6,m4,qtrans3)*V3NF_F8(Q32,Aq3,impi) )
!!$
!!$!321
!!$    vn3lo_2P1PE = vn3lo_2P1PE + sigma_dot_q_ope141*&
!!$         (tau_dot_tau_tab(t3,t1,t6,t4)*(chp_sigma_dot_q_mtx(m2,m5,qtrans3)*sum(qtrans3*qtrans1)*V3NF_F1(Q32,Aq3,impi) &
!!$         +chp_sigma_dot_q_mtx(m2,m5,qtrans3)*V3NF_F2(Q32,Aq3,impi) &
!!$         +chp_sigma_dot_q_mtx(m2,m5,qtrans1)*V3nf_F3(Q32,Aq3,impi))&
!!$         *delta_tab(m3,m6)*delta_tab(t2,t5) &
!!$         +tau_dot_tau_tab(t2,t1,t5,t4)*( (chp_sigma_dot_q_mtx(m3,m6,qtrans3)*sum(qtrans3*qtrans1)*V3NF_F4(Aq3) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3NF_F5(Q32,Aq3) )*delta(m2,m5)*delta(t3,t6) &
!!$         +(chp_sigma_dot_q_mtx(m2,m5,qtrans3)*V3NF_F6(Q32,Aq3,impi) &
!!$         +chp_sigma_dot_q_mtx(m2,m5,qtrans1)*V3NF_F7(Q32,Aq3,impi) )*delta(m3,m6)*delta(t3,t6)) &
!!$         +chp_tau1Xtau2_dot_tau3(t3,t2,t1,t6,t5,t4)*sigmaXsigma_dot_q(m3,m2,m6,m5,qtrans3)*V3NF_F8(Q32,Aq3,impi) )
!!$
!!$!231
!!$    vn3lo_2P1PE = vn3lo_2P1PE + sigma_dot_q_ope141*&
!!$         (tau_dot_tau_tab(t2,t1,t5,t4)*(chp_sigma_dot_q_mtx(m3,m6,qtrans2)*sum(qtrans2*qtrans1)*V3NF_F1(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3NF_F2(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3nf_F3(Q22,Aq2,impi))&
!!$         *delta_tab(m2,m5)*delta_tab(t3,t6) &
!!$         +tau_dot_tau_tab(t3,t1,t6,t4)*( (chp_sigma_dot_q_mtx(m2,m5,qtrans2)*sum(qtrans2*qtrans1)*V3NF_F4(Aq2) &
!!$         +chp_sigma_dot_q_mtx(m2,m5,qtrans1)*V3NF_F5(Q22,Aq2) )*delta(m3,m6)*delta(t2,t5) &
!!$         +(chp_sigma_dot_q_mtx(m3,m6,qtrans2)*V3NF_F6(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m3,m6,qtrans1)*V3NF_F7(Q22,Aq2,impi) )*delta(m2,m5)*delta(t2,t5)) &
!!$         +chp_tau1Xtau2_dot_tau3(t2,t3,t1,t5,t6,t4)*sigmaXsigma_dot_q(m2,m3,m5,m6,qtrans2)*V3NF_F8(Q22,Aq2,impi) )
!!$
!!$!213
!!$    vn3lo_2P1PE = vn3lo_2P1PE + sigma_dot_q_ope363*&
!!$         (tau_dot_tau_tab(t2,t3,t5,t6)*(chp_sigma_dot_q_mtx(m1,m4,qtrans2)*sum(qtrans2*qtrans3)*V3NF_F1(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans2)*V3NF_F2(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*V3nf_F3(Q22,Aq2,impi))&
!!$         *delta_tab(m2,m5)*delta_tab(t1,t4) &
!!$         +tau_dot_tau_tab(t1,t3,t4,t6)*( (chp_sigma_dot_q_mtx(m2,m5,qtrans2)*sum(qtrans2*qtrans3)*V3NF_F4(Aq2) &
!!$         +chp_sigma_dot_q_mtx(m2,m5,qtrans3)*V3NF_F5(Q22,Aq2) )*delta(m1,m4)*delta(t2,t5) &
!!$         +(chp_sigma_dot_q_mtx(m1,m4,qtrans2)*V3NF_F6(Q22,Aq2,impi) &
!!$         +chp_sigma_dot_q_mtx(m1,m4,qtrans3)*V3NF_F7(Q22,Aq2,impi) )*delta(m2,m5)*delta(t2,t5)) &
!!$         +chp_tau1Xtau2_dot_tau3(t2,t1,t3,t5,t4,t6)*sigmaXsigma_dot_q(m2,m1,m5,m4,qtrans2)*V3NF_F8(Q22,Aq2,impi) )
...
!!$
!!$
!!$    relcorr_1PE = 0.0; relcorr_2PE = 0.0;
!!$!    
!!$    relcorr_1PE = relcorr_1PE + rel1PE_3nf_corr(p,q,r,s,t,u)
...
!!$!    
!!$    relcorr_2PE = relcorr_2PE + rel2PE_3nf_corr(p,q,r,s,t,u)
...
!!    chiral_3nf = hbarc**6*(vn3lo_ring)/volume**2
